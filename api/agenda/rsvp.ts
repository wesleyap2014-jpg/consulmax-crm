...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!token || !['accepted', 'declined'].includes(response)) {
    return res.status(400).setHeader('Content-Type', 'text/html; charset=utf-8').send(page('Link inválido', 'Não foi possível identificar esta confirmação.'))
  }
  ...
  try {
    ...
    if (!guest) return res.status(404).setHeader('Content-Type', 'text/html; charset=utf-8').send(page('Convite não encontrado', 'Este convite não existe mais ou o link expirou.'))
    ...
    const title = accepted ? 'Presença confirmada' : 'Resposta registrada'
    ...
    return res.status(200).send(page(title, message, accepted ? (event?.videocall_url || event?.meeting_link) : null))
  } catch (err: any) {
    console.error('[agenda-rsvp]', err)
    return res.status(500).setHeader('Content-Type', 'text/html; charset=utf-8').send(page('Não foi possível confirmar', 'Ocorreu um erro ao registrar sua resposta. Tente novamente em alguns instantes.'))
  }
}
