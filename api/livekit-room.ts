...
function clean(raw: string) {
  return raw.toLowerCase().replace(/[^a-z0-9_-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 80)
}
...
async function createRoom(name: string) {
  const adminToken = token({ ttl: 600, video: { roomCreate: true, roomAdmin: true, room: name } })
  const response = await fetch(`${lkHttpUrl()}/twirp/livekit.RoomService/CreateRoom`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ name, emptyTimeout: 900, departureTimeout: 300 }),
  })
  if (!response.ok) {
    const text = await response.text().catch(() => '')
    if (!(response.status === 409 || /already exists|already_exist|exist/i.test(text))) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!publicAppUrl) return json(res, 500, { error: 'Falta PUBLIC_APP_URL na Vercel.' })

    const body = parseBody(req)
    const agendaEventoId = String(body?.agenda_evento_id || '').trim()
    const role: Role = body?.role === 'host' ? 'host' : 'client'
    const participantName = String(body?.participant_name || (role === 'host' ? 'Consultor Consulmax' : 'Cliente')).trim()

    if (!agendaEventoId) return json(res, 400, { error: 'agenda_evento_id é obrigatório.' })

    const userId = await authUserId(req)
    if (role === 'host' && !userId) return json(res, 401, { error: 'Usuário não autenticado para entrar como consultor.' })
    ...
    if (evento.video_status === 'finished') {
      return json(res, 409, { error: 'Esta videochamada já foi finalizada. Crie um novo evento para uma nova chamada.', finished: true })
    }
    ...
    if (room?.status === 'finished') {
      await admin.from('agenda_eventos').update({ video_status: 'finished' }).eq('id', agendaEventoId)
      return json(res, 409, { error: 'Esta videochamada já foi finalizada. Crie um novo evento para uma nova chamada.', finished: true })
    }
    ...
    if (!room) {
      ...
      const { data: inserted, error: insertError } = await admin
        .from('video_rooms')
        .insert({ agenda_evento_id: agendaEventoId, provider: 'livekit', provider_room_name: roomName, provider_room_url: LIVEKIT_WS_URL, public_client_url: clientUrl, status: 'created', created_by: userId })
        .select('*')
        .single()
      ...
      await admin.from('agenda_eventos').update({ video_room_id: inserted.id, videocall_url: clientUrl, video_status: 'created' }).eq('id', agendaEventoId)
      await admin.from('video_sessions').insert({ video_room_id: inserted.id, agenda_evento_id: agendaEventoId, status: 'scheduled' })
    }
    ...
    const participantToken = token({
      ...
      ttl: 7200,
      video: { room: room.provider_room_name, roomJoin: true, canPublish: true, canSubscribe: true, canPublishData: true },
    })

    await admin.from('video_rooms').update({ status: nextStatus, updated_at: new Date().toISOString() }).eq('id', room.id)
    await admin.from('agenda_eventos').update({ video_status: nextStatus }).eq('id', agendaEventoId)

    return json(res, 200, { ok: true, serverUrl: LIVEKIT_WS_URL, token: participantToken, room, clientUrl: room.public_client_url })
  } catch (err: any) {
