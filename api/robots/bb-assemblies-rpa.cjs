...
async function createBrowser() {
  ...
  if (isServerless) throw new Error('Chromium local indisponível no runtime da Vercel. Configure BROWSERLESS_TOKEN ou BROWSERLESS_WS_ENDPOINT.')
...
async function openAssemblyResult(page) {
  ...
  for (const candidate of candidates) {
    if (await candidate.isVisible().catch(() => false)) {
      await Promise.all([page.waitForLoadState('domcontentloaded').catch(() => null), candidate.click({ timeout: 10000 })])
...
async function searchGroup(page, grupo) {
  ...
  if (!groupNumber || groupNumber === '000000') throw new Error(`Grupo inválido para resultado de assembleia: ${grupo}`)
...
async function readLatestAssembly(page, grupo) {
  const rows = await page.locator('table tr').evaluateAll((trs) => trs.map((tr) => Array.from(tr.querySelectorAll('td')).map((td) => String(td.innerText || td.textContent || '').trim())).filter((cells) => cells.length >= 6 && /^\d+/.test(cells[0] || '') && /^\d+/.test(cells[1] || '')))
...
async function updateGroupAssembly(supabase, result) {
  ...
  let { data: rows, error: findError } = await supabase.from('sim_bb_groups').select('id,grupo,config').eq('grupo', padded)
  if (findError) throw findError
  if (!rows?.length) {
    const fallback = await supabase.from('sim_bb_groups').select('id,grupo,config').eq('grupo', grupo)
  ...
  for (const row of rows || []) {
    ...
    const { error } = await supabase.from('sim_bb_groups').update({ config: { ...config, assemblyResult } }).eq('id', row.id)
...
module.exports = { syncBBAssemblyResultRpa }
