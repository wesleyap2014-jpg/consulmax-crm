...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const cpf: string | undefined = body?.cpf ? String(body.cpf) : undefined; // dígitos
    ...
    //    IMPORTANTE: se você tiver triggers/RLS que criptografam CPF em cpf_encrypted,
    //    pode enviar "cpf" para uma função RPC própria. Aqui gravamos campos visíveis/seguros.
    const profile = {
      auth_user_id,
      nome,
      email,
    ...
    if (upsertErr) {
      // Mesmo que o upsert falhe, o usuário foi criado no Auth.
      // Enviamos erro para você corrigir a tabela/RLS.
