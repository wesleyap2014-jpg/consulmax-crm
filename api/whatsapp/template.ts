...
const DEFAULT_PHONE_NUMBER_ID =
  process.env["META" + "_WHATSAPP" + "_PHONE" + "_NUMBER" + "_ID"]!;
const WABA_ID =
  process.env["META" + "_WHATSAPP" + "_WABA" + "_ID"] ||
  process.env["META" + "_WABA" + "_ID"] ||
  process.env["WHATSAPP" + "_BUSINESS" + "_ACCOUNT" + "_ID"] ||
  "";
...
const BIRTHDAY_IMAGE_URL =
  process.env.WHATSAPP_BIRTHDAY_IMAGE_URL ||
  process.env.VITE_WHATSAPP_BIRTHDAY_IMAGE_URL ||
  "";
...
function normalizeLanguage(value?: string | null) {
  const v = String(value || "pt_BR").trim();
  return v === "pt-br" ? "pt_BR" : v;
}
...
function safeFileName(value?: string | null) {
  return String(value || "arquivo.pdf")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function mediaTypeFromMime(mimeType?: string | null, explicit?: string | null) {
  const requested = String(explicit || "").toLowerCase();
  if (["image", "video", "document"].includes(requested)) return requested;
...
async function resolveWabaId() {
  ...
  const phone = await metaGet(DEFAULT_PHONE_NUMBER_ID, {
...
async function getTemplateDefinition(name: string, language: string) {
  ...
  return (
    rows.find((t: any) => t.name === name && t.language === language) ||
    rows.find((t: any) => t.name === name) ||
    null
  );
}
...
async function getConversationContact(conversationId: string) {
  const { data } = await supabaseAdmin
    ...
    .maybeSingle();
  const contact: any = Array.isArray((data as any)?.whatsapp_contacts)
    ? (data as any).whatsapp_contacts[0]
    : (data as any)?.whatsapp_contacts;
  return contact || {};
}

function plainTextFromParam(value: any) {
  return String(value?.text ?? value?.payload ?? value ?? "").trim();
}
function makeParam(name: string, value: any) {
  const param: any = {
    type: "text",
    text: String(value ?? "").trim() || "Cliente",
  };
...
function componentByType(templateDefinition: any, type: string) {
  return (templateDefinition?.components || []).find(
    (c: any) => String(c?.type || "").toUpperCase() === type.toUpperCase(),
  );
}
function bodyComponent(templateDefinition: any) {
  return componentByType(templateDefinition, "BODY");
}
function headerComponent(templateDefinition: any) {
  return componentByType(templateDefinition, "HEADER");
}
function bodyVariableCount(templateDefinition: any) {
  return countVars(bodyComponent(templateDefinition)?.text || "");
}
...
function normalizeProvidedParams(raw: any[]) {
  ...
  for (const item of arr) {
    if (item && typeof item === "object" && !Array.isArray(item)) {
      ...
      const key = normalizeVarKey(
        item.name || item.parameter_name || item.key || "",
      );
...
async function uploadTemplateHeaderMedia(conversationId: string, reqBody: any) {
  const fileBase64 =
    reqBody?.file_base64 || reqBody?.header_file_base64 || null;
  const mimeType = String(
    reqBody?.mime_type || reqBody?.header_mime_type || "application/pdf",
  );
  if (!fileBase64) return null;

  const cleanFileName = safeFileName(
    reqBody?.file_name || reqBody?.header_file_name || "boleto.pdf",
  );
  const mediaType = mediaTypeFromMime(
    mimeType,
    reqBody?.media_type || "document",
  );
  const base64 = String(fileBase64).includes(",")
  ...
  if (saved.error)
    throw new Error(saved.error.message || "Erro ao salvar anexo.");
  ...
  if (!uploadResponse.ok || !uploadData?.id)
    throw new Error(
      JSON.stringify(uploadData || {}).slice(0, 800) ||
        "Erro ao subir anexo para a Meta.",
    );
...
function buildHeaderComponent(
  ...
  const isBirthdayTemplate =
    String(templateName || "") === BIRTHDAY_TEMPLATE_NAME;
  const media = reqBody?.header_media || reqBody?.media || null;
  const birthdayImageLink =
    isBirthdayTemplate && format === "image" ? BIRTHDAY_IMAGE_URL : null;
  const link =
    reqBody?.header_media_link ||
    reqBody?.media_link ||
    reqBody?.document_link ||
    reqBody?.image_link ||
    media?.link ||
    birthdayImageLink ||
    null;
  ...
  const filename =
    reqBody?.header_file_name ||
    reqBody?.file_name ||
    media?.filename ||
    media?.file_name ||
    undefined;
  const type = ["image", "document", "video"].includes(format)
    ? format
    : String(media?.type || reqBody?.media_type || "").toLowerCase();
  ...
  try {
    ...
    if (!conversation_id || !phone || !name)
      return res
        ...
        .json({
          ok: false,
          error: "conversation_id, to e template_name são obrigatórios.",
        });
    ...
    const headerFormat = String(
      headerComponent(templateDefinition)?.format || "",
    ).toUpperCase();
    if (
      ...
      return res
        ...
        .json({
          ok: false,
          error:
            "O modelo de aniversário possui cabeçalho de imagem, mas WHATSAPP_BIRTHDAY_IMAGE_URL não está configurada na Vercel.",
        });
    }

    const uploadedHeaderMedia = await uploadTemplateHeaderMedia(
      conversation_id,
      req.body || {},
    );
    const requestBodyForHeader = uploadedHeaderMedia
      ? {
          ...(req.body || {}),
          header_media: uploadedHeaderMedia,
          header_media_id: uploadedHeaderMedia.id,
          header_file_name: uploadedHeaderMedia.filename,
          media_type: uploadedHeaderMedia.type,
        }
      : req.body || {};

    const needed = bodyVariableCount(templateDefinition);
    const provided = Array.isArray(template_params)
      ? template_params
      : Array.isArray(body_params)
        ? body_params
        : [];
    const templatePayload: any = { name, language: { code: language } };
    const components: any[] = [];

    const headerPayload = buildHeaderComponent(
      templateDefinition,
      requestBodyForHeader,
      name,
    );
    if (headerPayload)
      components.push({ type: "header", parameters: headerPayload.parameters });

    const bodyParams =
      needed > 0
        ? await buildBodyParams(conversation_id, templateDefinition, provided)
        : [];
    ...
    if (mediaInfo?.storage_path) {
      rawPayload._consulmax_media = {
        ...
        original_file_name:
          mediaInfo.original_file_name || mediaInfo.filename || "boleto.pdf",
      };
    }
    ...
    await supabaseAdmin
      ...
      .insert({
        ...
        message_type: mediaInfo?.type || "template",
        body,
    ...
    return res
      ...
      .json({
        ...
        template_debug: {
          ...
          needed,
          template_found: !!templateDefinition,
          used_components: templatePayload.components || [],
          media: mediaInfo,
        },
      });
  } catch (error: any) {
    ...
    return res
      .status(500)
      .json({ ok: false, error: error?.message || "Erro ao enviar modelo." });
  }
}
