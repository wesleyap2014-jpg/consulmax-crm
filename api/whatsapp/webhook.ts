...
async function findWhatsAppContactByAliases(phone: string) {
  ...
  const byTel = await supabaseAdmin
    .from("whatsapp_contacts")
    .select("id,lead_id,wa_id,telefone,nome")
...
function isRatingResponse(body?: string | null) {
  ...
  return (
    !!digits && digits.length === 1 && /[1-5]/.test(digits) && raw.length <= 20
  );
}
...
function isRecentClosedConversation(value?: string | null) {
  ...
  return (Date.now() - closedAt) / (1000 * 60 * 60 * 24) <= 7;
}
...
function isOptOutMessage(body?: string | null) {
  ...
  return (
    /\b(sair|parar|cancelar|descadastrar|descadastro|stop|unsubscribe|remover)\b/i.test(
      text,
    ) ||
...
function extractCallId(call: any) {
  return (
    call?.id || call?.call_id || call?.meta_call_id || call?.call?.id || null
  );
}
function extractCallStatus(call: any) {
  return String(
    call?.event || call?.status || call?.type || call?.state || "received",
  ).toLowerCase();
}
...
async function handleMessageStatus(statusEvent: any) {
  const metaMessageId = statusEvent?.id || null;
  const nextStatus = String(statusEvent?.status || "").toLowerCase();
  if (!metaMessageId || !MESSAGE_STATUS_RANK[nextStatus]) return;
  ...
  if (error) {
    console.error("WHATSAPP_STATUS_LOOKUP_ERROR", error);
    return;
  }
  if (!message?.id) {
    console.warn("WHATSAPP_STATUS_MESSAGE_NOT_FOUND", {
      metaMessageId,
      nextStatus,
    });
    return;
  ...
  if (currentStatus === "failed" && nextStatus !== "failed") return;

  if (
    nextStatus !== "failed" &&
    MESSAGE_STATUS_RANK[nextStatus] < (MESSAGE_STATUS_RANK[currentStatus] || 0)
  )
    return;
  ...
  const history = Array.isArray(message.raw_payload?.meta_status_history)
    ? message.raw_payload.meta_status_history
    : [];
  const historyEntry = { status: nextStatus, at: statusAt };
  const nextHistory = [
    ...history.filter((item: any) => item?.status !== nextStatus),
    historyEntry,
  ].slice(-8);
  const metaError =
    nextStatus === "failed"
      ? statusEvent?.errors?.[0] || { message: "Falha informada pela Meta." }
      : null;
  ...
  const { error: updateError } = await supabaseAdmin
    ...
    .update({
      raw_payload: {
        ...
        meta_status_at: statusAt,
        meta_status_history: nextHistory,
        meta_status_event: statusEvent,
        meta_error: metaError,
      },
    })
    .eq("id", message.id);

  if (updateError) console.error("WHATSAPP_STATUS_UPDATE_ERROR", updateError);
}

async function resolveReplyMetadata(message: any) {
  ...
  return {
    meta_message_id: replyToMetaMessageId,
...
async function registerOptOut(params: {
  ...
  if (!phone) return;
  const { error } = await supabaseAdmin.from("whatsapp_opt_outs").upsert(
    {
      telefone_digits: phone,
      nome: params.nome || null,
      reason: params.body || "Opt-out via WhatsApp",
      source: params.source || "whatsapp_inbound",
...
async function removeOptOut(params: { telefoneDigits: string }) {
  ...
  if (error) console.error("WHATSAPP_OPT_OUT_DELETE_ERROR", error);
}
...
async function findActiveConversation(contactId: string) {
  const { data, error } = await supabaseAdmin
    ...
    .not("stage", "in", "(finalizado)")
    .not("queue", "in", "(finalizado,fechado_ganho,fechado_perdido)")
...
async function findRecentlyClosedConversationForRating(contactId: string) {
  const { data, error } = await supabaseAdmin
    ...
    .eq("contact_id", contactId)
    .or(
      "status.eq.fechada,status.eq.finalizado,stage.eq.finalizado,queue.eq.finalizado",
    )
    .order("last_message_at", { ascending: false, nullsFirst: false })
    .limit(1)
    .maybeSingle();
...
async function handleSingleInboundMessage(
  ...
  const contactFromContacts =
    value?.contacts?.find((c: any) => c?.wa_id === contactFromMessage) ||
    value?.contacts?.[0];
  ...
  const contactResult = existingContact?.id
    ...
    : await supabaseAdmin
        .from("whatsapp_contacts")
        .upsert(
          { wa_id: waId, telefone: waId, nome, updated_at: inboundAt },
          { onConflict: "wa_id" },
        )
        .select("id, lead_id")
        .single();
  ...
  if (!conversation?.id && isRating) {
    const closedConversation = await findRecentlyClosedConversationForRating(
      contact.id,
    );
  ...
  const storedMedia = mediaId
    ...
    : null;
  const replyMetadata = await resolveReplyMetadata(message);
  const rawPayloadWithMedia = {
    ...payload,
    ...(storedMedia ? { _consulmax_media: storedMedia } : {}),
    ...(replyMetadata ? { _reply_to: replyMetadata } : {}),
  };
  const messagePayload = {
    conversation_id: conversation.id,
    direction: "inbound",
    sender_type: handledAsClosedRating
      ? "avaliacao"
      : optOut
        ? "opt_out"
        : optIn
  ...
  if (optOut) {
    ...
    console.log("WHATSAPP_OPT_OUT_REGISTERED", {
      waId,
      conversationId: conversation.id,
    });
    return;
  ...
  if (optIn) {
    ...
    console.log("WHATSAPP_OPT_IN_REGISTERED", {
      waId,
      conversationId: conversation.id,
    });
    return;
...
async function handleSingleCallEvent(payload: any, value: any, call: any) {
  ...
  const direction =
    call?.from || call?.caller || call?.customer ? "inbound" : "outbound";
  ...
  const { error: insertCallError } = await supabaseAdmin
    .from("whatsapp_calls")
    .insert({
      phone: waId || null,
      wa_id: waId || null,
    ...
    for (const change of changes) {
      ...
      if (Array.isArray(value?.statuses) && value.statuses.length > 0) {
        ...
        for (const statusEvent of value.statuses)
          await handleMessageStatus(statusEvent);
      }
