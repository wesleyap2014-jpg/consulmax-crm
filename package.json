...
export default function ComissoesPage() {
  ...
  async function openPaymentFor(c: Commission) {
    ...
    // cálculo correto para exibição: comissão total * % parcela
  ...
  async function paySelectedParcels(payload: {
    ...
    if (!selected.length)
      return alert(
        "Selecione pelo menos uma parcela (ou informe a mesma data do pagamento ao anexar arquivos).",
      );

    // UPDATE por id (preferível) — NUNCA pisar valor pago sem input
        ...
        const { error } = await supabase
          ...
          .update({
            data_pagamento_vendedor:
              payload.data_pagamento_vendedor || f.data_pagamento_vendedor || toDateInput(new Date()),
            valor_pago_vendedor:
    ...
    // INSERT (apenas quando não há id) — sem pagar automaticamente
    ...
    const allPaid =
      (updated || []).length > 0 && (updated || []).every((f: any) => (f.valor_pago_vendedor ?? 0) > 0);
    if (allPaid) {
      await supabase
        .from("commissions")
    ...
    setPayFlow(freshArr as any);
    setRows((prev) =>
      prev.map((r) => (r.id === payCommissionId ? { ...r, flow: freshArr, status: allPaid ? "pago" : r.status } : r)),
    );
    setOpenPay(false);
    fetchData();
  }
  ...
  async function gerarComissaoDeVenda(venda: Venda) {
    try {
      ...
      if (!vendedorIdCanon) {
        alert("Vendedor desta venda não está cadastrado em 'users' (vínculo por auth_user_id).");
        return;
      }
  ...
  async function retornarComissao(c: Commission) {
    ...
    try {
      ...
      if (stillFlows && stillFlows.length > 0) {
        alert(
          "Não foi possível remover as parcelas (RLS). Ajuste as policies de 'commission_flow' para permitir DELETE pelo owner/admin.",
        );
        return;
      }
  ...
  async function downloadReceiptPDFPorData() {
    ...
    const totalLinhas = commsFiltradas.reduce(
      (acc, c: any) => acc + new Map((byCommission[c.id] || []).map((p) => [p.mes, p])).size,
      0,
    );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs text-gray-500">
        Arquivos vão para o bucket <code>comissoes</code>. Se nenhuma parcela estiver marcada, a confirmação
        aplica aos pagamentos cuja <b>data</b> coincide com a informada; se ainda assim nada combinar e existir
        apenas 1 parcela paga, ela será selecionada automaticamente.
      </div>
    </div>
  );
}
