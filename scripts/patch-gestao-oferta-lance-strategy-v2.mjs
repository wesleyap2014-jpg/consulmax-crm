...
if (!src.includes('function formatLanceStrategy')) {
  if (!src.includes(helperAnchor)) throw new Error('patch-gestao-oferta-lance-strategy-v2: helper anchor not found');
...
if (!src.includes('estrategia_lance: string;')) {
  if (!src.includes(typeOld)) throw new Error('patch-gestao-oferta-lance-strategy-v2: OfertaRow anchor not found');
  src = src.replace(typeOld, typeNew);
}

const descBlock = lines([
...
if (src.includes(descBlock)) {
  src = src.replace(descBlock, '      const leadKey: string | null = leadCandidates.find((k) => k in sample) ?? null;\n');
}
...
if (src.includes(pushOld)) src = src.replace(pushOld, pushNew);

const pdfCssOld = lines([
...
if (src.includes(pdfHeadOld)) src = src.replace(pdfHeadOld, pdfHeadNew);

const scriptOld = lines([
  '          <script>',
...
console.log('patch-gestao-oferta-lance-strategy-v2 applied');
