...
const oldBlock = `      const updatePayload = handledAsClosedRating
        ? {
...
const newBlock = `      const inboundStatus = String(conversation.status || "").toLowerCase();
...
const oldAfterUpdate = `    if (updateConversationError) {
      console.error("WHATSAPP_CONVERSATION_UPDATE_ERROR", updateConversationError);
    }`;

const newAfterUpdate = `    if (updateConversationError) {
      console.error("WHATSAPP_CONVERSATION_UPDATE_ERROR", updateConversationError);
    }
