...
if (!fs.existsSync(file)) {
  console.log("[patch-whatsapp-template-modal-v40] arquivo não encontrado");
  process.exit(0);
}
...
function log(label, status) {
  console.log(`[patch-whatsapp-template-modal-v40] ${label}: ${status}`);
}
...
const finalTemplateBlock = `  function normalizeTemplateVarKey(value?: string | null) {
...
// Remove qualquer bloco antigo de helpers de template que patches anteriores tenham criado.
src = src.replace(/\n\s*function normalizeTemplateVarKey\([\s\S]*?\n\s*async function finishConversation/, "\n" + finalTemplateBlock + "  async function finishConversation");
src = src.replace(/\n\s*function templateVariableValues\([\s\S]*?\n\s*async function finishConversation/, "\n" + finalTemplateBlock + "  async function finishConversation");

if (!src.includes("normalizeTemplateVarKey")) {
  const re = /\n\s*async function sendTemplate\(conv: Conv\) \{[\s\S]*?\n\s*async function finishConversation/;
  if (re.test(src)) {
    src = src.replace(re, "\n" + finalTemplateBlock + "  async function finishConversation");
    changed = true;
    log("sendTemplate final", "aplicado");
  } else {
    log("sendTemplate final", "não encontrado");
  }
} else {
  changed = true;
  log("sendTemplate final", "normalizado");
}

// Garante envio de template ao criar novo ticket quando um modelo estiver selecionado.
src = src.replace(
...
// Garante renderização de mídia/template quando v39 não tiver conseguido substituir o balão.
if (!src.includes("function getMessageMedia")) {
  const mediaBlock = `  function getMessageMedia(m: Msg): any {
...
`;
  if (src.includes("function Row({ c }: { c: Conv })")) {
    src = src.replace("function Row({ c }: { c: Conv })", mediaBlock + "  function Row({ c }: { c: Conv })");
    changed = true;
    log("renderMessageBody", "inserido");
  }
}
...
console.log("[patch-whatsapp-template-modal-v40] concluído");
