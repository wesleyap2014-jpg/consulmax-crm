...
const PARAM_STORAGE_KEY = "proposalParamsV4";
...
function getAdminName(row: Proposal) {
  ...
  const raw = normalizeText(Object.values(row).join(" "));
  if (raw.includes("bb consorcios") || raw.includes("bbconsorcios") || raw.includes("banco do brasil")) return "BB Consórcios";
  if (raw.includes("maggi")) return "Maggi";
  if (raw.includes("embracon")) return "Embracon";

  // A guia Propostas original nasce de sim_simulations, que hoje concentra as simulações Embracon.
  return "Embracon";
}
...
export default function PropostasProMax() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(PARAM_STORAGE_KEY, JSON.stringify(proposalParams));
    } catch {}
  }, [proposalParams]);
  ...
  function updateProposalParam<K extends keyof ProposalParams>(key: K, value: ProposalParams[K]) {
    setProposalParams((prev) => ({ ...prev, [key]: value }));
  }
        ...
        {paramsOpen && (
          <section className="rounded-xl border bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              ...
              <Button type="button" variant="outline" className="rounded-lg" onClick={() => setProposalParams(DEFAULT_PARAMS)}>
                Restaurar padrão
              </Button>
            </div>
