...
type LogoFormat = "vertical" | "horizontal" | "icon" | "variation";
type BackgroundContext = "light" | "dark" | "any";
...
const LOGO_FORMATS: Array<[LogoFormat, string]> = [
  ["vertical", "Vertical"],
  ["horizontal", "Horizontal"],
  ["icon", "Ícone"],
  ["variation", "Outra variação"],
];

const BACKGROUND_CONTEXTS: Array<[BackgroundContext, string]> = [
  ["light", "Fundo claro"],
  ["dark", "Fundo escuro"],
  ["any", "Qualquer fundo"],
];
...
function labelFrom<T extends string>(options: Array<[T, string]>, value: string) {
  return options.find(([key]) => key === value)?.[1] || value;
}

function logoFormatValue(asset: BrandAsset): LogoFormat {
  const fromMetadata = asset.metadata?.logo_format;
  if (["vertical", "horizontal", "icon", "variation"].includes(fromMetadata)) return fromMetadata as LogoFormat;
  if (["vertical", "horizontal", "icon", "variation"].includes(asset.role)) return asset.role as LogoFormat;
  if (asset.role === "logo_horizontal") return "horizontal";
  if (asset.role === "simbolo") return "icon";
  if (asset.role === "variacao") return "variation";
  return "vertical";
}

function backgroundContextValue(asset: BrandAsset): BackgroundContext {
  const value = asset.metadata?.background_context;
  return value === "light" || value === "dark" || value === "any" ? value : "any";
}

function fontRoleLabel(asset: BrandAsset) {
  return FONT_ROLES.find(([key]) => key === asset.role)?.[1] || asset.role;
}

export default function BrandKitAssetsV2({ settingId, userId }: Props) {
  const [assets, setAssets] = useState<BrandAsset[]>([]);
  const [previewUrls, setPreviewUrls] = useState<Record<string, string>>({});
  const [logoFormat, setLogoFormat] = useState<LogoFormat>("vertical");
  const [logoBackground, setLogoBackground] = useState<BackgroundContext>("light");
  ...
  async function upload(type: AssetType, files: FileList | null) {
    ...
    try {
      for (let index = 0; index < files.length; index += 1) {
        ...
        const role = type === "logo" ? logoFormat : fontRole;
        const metadata: Record<string, any> = {
          extension: extension(file.name),
          source: "brand_kit_upload",
        };
        if (type === "logo") {
          metadata.logo_format = logoFormat;
          metadata.background_context = logoBackground;
        }
  ...
  async function changeLogoFormat(asset: BrandAsset, format: LogoFormat) {
    setError(null);
    const metadata = { ...(asset.metadata || {}), logo_format: format };
    const { error: updateError } = await supabase
      .from("marketing_brand_assets")
      .update({ role: format, metadata, updated_at: new Date().toISOString() })
      .eq("id", asset.id);
    if (updateError) return setError(updateError.message);
    await loadAssets();
  }

  async function changeLogoBackground(asset: BrandAsset, background: BackgroundContext) {
    setError(null);
    const metadata = { ...(asset.metadata || {}), background_context: background };
    const { error: updateError } = await supabase
      .from("marketing_brand_assets")
  ...
  async function changeFontRole(asset: BrandAsset, role: string) {
    setError(null);
    const { error: updateError } = await supabase
      .from("marketing_brand_assets")
    ...
    <div className="space-y-4 rounded-2xl border border-[#B5A573]/30 bg-white p-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-slate-500">
            Cada logo é identificada pelo formato e pelo fundo onde deve ser aplicada. O Max deve usar essa combinação como fonte de verdade e nunca redesenhar a marca com IA.
          </p>
        </div>
      </div>
        ...
        <div className="rounded-xl border border-slate-200 p-3">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <label className="text-[11px] font-medium text-slate-500">
              Formato
              <select className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-xs" value={logoFormat} onChange={(event) => setLogoFormat(event.target.value as LogoFormat)}>
                {LOGO_FORMATS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
            </label>
            <label className="text-[11px] font-medium text-slate-500">
              Aplicação
              <select className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-xs" value={logoBackground} onChange={(event) => setLogoBackground(event.target.value as BackgroundContext)}>
                {BACKGROUND_CONTEXTS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
              </select>
            </label>
          </div>
          ...
          <p className="mt-2 text-[11px] text-slate-400">Escolha formato + fundo antes do upload. PNG, JPG, WEBP ou SVG · até 25 MB.</p>
        </div>
      ...
      {logos.length ? (
        <div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {logos.map((asset) => {
              const format = logoFormatValue(asset);
              const background = backgroundContextValue(asset);
              const darkPreview = background === "dark";
              return (
                <div key={asset.id} className={`rounded-xl border p-3 ${asset.active ? "border-slate-200" : "border-dashed border-slate-200 opacity-60"}`}>
                  <div className="flex gap-3">
                    <div className={`flex h-16 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg border ${darkPreview ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1">
                        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-600">{labelFrom(LOGO_FORMATS, format)}</span>
                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${background === "dark" ? "bg-slate-900 text-white" : background === "light" ? "border border-slate-200 bg-white text-slate-700" : "bg-slate-100 text-slate-600"}`}>{labelFrom(BACKGROUND_CONTEXTS, background)}</span>
                        {asset.is_primary ? <span className="rounded-full bg-[#E0CE8C]/30 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#1E293F]">Principal</span> : null}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <label className="text-[10px] font-medium uppercase tracking-wide text-slate-400">
                      Formato
                      <select className="mt-1 h-8 w-full rounded-md border border-input bg-background px-2 text-[11px]" value={format} onChange={(event) => changeLogoFormat(asset, event.target.value as LogoFormat)}>
                        {LOGO_FORMATS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                      </select>
                    </label>
                    <label className="text-[10px] font-medium uppercase tracking-wide text-slate-400">
                      Aplicação
                      <select className="mt-1 h-8 w-full rounded-md border border-input bg-background px-2 text-[11px]" value={background} onChange={(event) => changeLogoBackground(asset, event.target.value as BackgroundContext)}>
                        {BACKGROUND_CONTEXTS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                      </select>
                    </label>
                  </div>
              ...
              <div key={asset.id} className={`flex flex-col gap-2 rounded-xl border p-3 sm:flex-row sm:items-center ${asset.active ? "border-slate-200" : "border-dashed border-slate-200 opacity-60"}`}>
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-xs font-medium text-[#1E293F]" title={asset.file_name}>{asset.file_name}</p>
                    <p className="text-[11px] text-slate-400">{fontRoleLabel(asset)}{asset.file_size_bytes ? ` · ${prettySize(asset.file_size_bytes)}` : ""}</p>
                  </div>
                </div>
                <select className="h-8 rounded-md border border-input bg-background px-2 text-[11px]" value={asset.role} onChange={(event) => changeFontRole(asset, event.target.value)}>
                  {FONT_ROLES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}
                </select>
      ...
      {!loading && !logos.length && !fonts.length ? (
        <div className="rounded-xl border border-dashed border-slate-200 px-4 py-5 text-center text-xs text-slate-500">
          Nenhum arquivo oficial cadastrado ainda. Envie primeiro uma logo, defina formato + fundo e depois cadastre as demais variações e fontes.
        </div>
      ) : null}
    </div>
  );
