...
type AppUser = {
  ...
  email?: string | null;
  role?: string | null;
  auth_user_id?: string | null; // supabase auth uid
};
    ...
    <tr className="border-t">
      ...
      <td className="p-2">
        <input
          ...
          placeholder={!canEncarteirar ? "Admin" : ""}
        />
      </td>
      <td className="p-2">
        <input
      ...
      <td className="p-2">
        <div className="flex gap-2">
          <button
            ...
            title={!canEncarteirar ? "Somente admin encarteira" : "Encarteirar"}
          >
            ENCARTEIRAR
          </button>
        ...
        {!canEncarteirar && (
          <div className="text-xs text-gray-400 mt-1">Somente admin pode encarteirar.</div>
        )}
      </td>
    </tr>
  );
...
const Carteira: React.FC = () => {
  ...
  const [selectedSeller, setSelectedSeller] = useState<string>(""); // admin: users.id, vendedor: fixado
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // selectedSeller = users.id (admin: todos, vendedor: fixado)
  ...
  // tabelas filtradas por Administradora + Segmento
  ...
  // Produto condicionado à Administradora
  ...
  const loadMetrics = async (sellerId: string, year: number): Promise<void> => {
    ...
    // ===== Realizado: encarteiradas ativas - canceladas (por ano) =====
    ...
    // vendedor_id na vendas = auth_user_id
  ...
  const saveMeta = async () => {
    try {
      ...
      const userRow = users.find((u) => u.id === metaForm.vendedor_id);
      const authUid = userRow?.auth_user_id ?? null;

      const payload: any = {
        vendedor_id: metaForm.vendedor_id,
        auth_user_id: authUid, // ✅ agora existe no schema
        ano: metaForm.ano,
        m01: metaForm.m[0],
        m02: metaForm.m[1],
        m03: metaForm.m[2],
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* ✅ Só admin pode cadastrar/editar meta */}
          {isAdmin && (
            ...
            <tbody>
              ...
              {pendentesComNome.map(({ venda, lead }) => (
                <LinhaEncarteirar
                  ...
                  venda={venda}
                  lead={lead}
                  canEncarteirar={isAdmin} // ✅ vendedor não encarteira
