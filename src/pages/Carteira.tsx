...
/** ===================== Toast + Confirm (sem alert/confirm) ===================== */
type ToastState = { open: boolean; message: string; variant?: "info" | "error" | "success" };
...
type LinhaEncarteirarProps = {
  ...
  onAskDelete: (vendaId: string) => void;
};
...
const Carteira: React.FC = () => {
  ...
  const notify = (message: string, variant: ToastState["variant"] = "info") => {
    setToast({ open: true, message, variant });
    window.setTimeout(() => setToast((t) => ({ ...t, open: false })), 2600);
  };

  const askConfirm = (p: Omit<ConfirmState, "open">) => {
    setConfirmState({ open: true, ...p });
  };
        ...
        const [{ data: lds }, pend, enc, { data: admins }, { data: tables }, { data: us }] =
          await Promise.all([
            ...
            (async () => {
              ...
              if (!adminFlag) qb = qb.eq("vendedor_id", uid);
              const { data } = await qb;
              return data;
            })(),
            (async () => {
  ...
  const registrarVenda = async () => {
    try {
      ...
      let pendQuery = supabase.from("vendas").select("*").eq("status", "nova").order("created_at", { ascending: false });
      if (!isAdmin) pendQuery = pendQuery.eq("vendedor_id", userId);
      ...
      notify(e.message ?? "Erro ao registrar venda.", "error");
    }
  };
  ...
  const encarteirar = async (vendaId: string, grupo: string, cota: string, codigo: string) => {
    try {
      ...
      const [{ data: pend }, { data: enc }] = await Promise.all([
        (async () => {
          let qb = supabase.from("vendas").select("*").eq("status", "nova").order("created_at", { ascending: false });
          if (!isAdmin) qb = qb.eq("vendedor_id", userId);
          return await qb;
        })(),
        (async () => {
          let qb = supabase.from("vendas").select("*").eq("status", "encarteirada").order("created_at", { ascending: false });
          if (!isAdmin) qb = qb.eq("vendedor_id", userId);
          return await qb;
        })(),
      ]);
      ...
      notify("Encarteirada ✅", "success");
    } catch (e: any) {
      notify(e.message ?? "Erro ao encarteirar.", "error");
    }
  };
  ...
  const reloadEncarteiradas = async () => {
    let encQuery = supabase.from("vendas").select("*").eq("status", "encarteirada").order("created_at", { ascending: false });
    if (!isAdmin) encQuery = encQuery.eq("vendedor_id", userId);
  ...
  const salvarEdicaoPendente = async (venda: Venda, novo: Partial<Venda>) => {
    try {
      ...
      notify("Venda atualizada ✅", "success");
    } catch (e: any) {
  ...
  const handleTransferSave = async () => {
    try {
      ...
      notify(e.message ?? "Erro ao transferir cota.", "error");
    }
  };
  ...
  const loadMetrics = async (sellerId: string, year: number): Promise<void> => {
    ...
    if (sellerId) {
      ...
      ativasBase = ativasBase.eq("vendedor_id", authIdToFilter);
      cancBase = cancBase.eq("vendedor_id", authIdToFilter);
    }

    const [{ data: vendasAtivas }, { data: vendasCanc }] = await Promise.all([ativasBase, cancBase]);
  ...
  const handleOpenMeta = () => {
    if (!users || users.length === 0) {
      notify("Aguarde carregar os vendedores antes de cadastrar a meta.", "info");
      return;
    }
  ...
  const saveMeta = async () => {
    try {
      ...
      notify(e.message || "Erro ao salvar metas.", "error");
    }
  };
  ...
  const saveCotaCodigo = async () => {
    try {
      ...
      closeCotaEditor();
      notify("Cota atualizada ✅", "success");
    } catch (e: any) {
      notify(e.message ?? "Erro ao salvar alterações de cota.", "error");
    }
  };
  ...
  const saveContemplacao = async () => {
    try {
      ...
      if (!ceContFlag) {
        ...
        const pctNum = parsePct4(ceContPctRaw);
        if (pctNum == null) throw new Error("Informe o % do lance (ex.: 41,2542%).");

        patch.contemplada = true;
        patch.data_contemplacao = ceContDate;
        patch.contemplacao_tipo = ceContTipo;
        patch.contemplacao_pct = Number(pctNum.toFixed(4));
      }
      ...
      closeCotaEditor();
      notify("Contemplação salva ✅", "success");
    } catch (e: any) {
      notify(e.message ?? "Erro ao salvar contemplação.", "error");
    }
  };
  ...
  const saveInad = async () => {
    try {
      ...
      closeCotaEditor();
      notify("Inadimplência salva ✅", "success");
    } catch (e: any) {
      notify(e.message ?? "Erro ao salvar inadimplência.", "error");
    }
  };
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center">
          <div className="bg-white w-full max-w-md rounded-2xl p-5 space-y-4">
            <div>
              ...
              <div className="text-sm text-gray-600 mt-1">{confirmState.message}</div>
            </div>
            <div className="flex items-center justify-end gap-2">
              <button className="px-4 py-2 rounded-xl border" onClick={closeConfirm}>
                {confirmState.cancelText ?? "Cancelar"}
              </button>
              <button
                className={`px-4 py-2 rounded-xl text-white hover:opacity-90 ${
                  confirmState.destructive ? "bg-[#A11C27]" : "bg-[#1E293F]"
                }`}
                onClick={async () => {
      ...
      {toast.open && (
        <div className="fixed bottom-5 right-5 z-[70]">
          <div
            className={`px-4 py-3 rounded-2xl shadow-lg border bg-white text-sm ${
              toast.variant === "error"
                ? "border-red-200"
