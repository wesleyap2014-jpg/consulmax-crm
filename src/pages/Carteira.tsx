...
const TABELAS: Record<Produto, string[]> = {
  ...
  "Imóvel Estendido": ["Select Estendido", "Super Crédito Estendido", "Plano Estendido Im", "Select Estendido Prime"],
  "Consórcio Ouro": [],
};
...
/** Linha de pendente para encarteirar */
    ...
    <tr className="border-t">
      <td className="p-2">
        <div className="flex items-center gap-2">
          ...
          <button title="Ver/Editar venda" className="text-gray-500 hover:text-gray-800" onClick={() => onOpenEditarVenda(venda)}>
            👁️
          </button>
        </div>
      ...
      <td className="p-2">
        <div className="flex gap-2">
          ...
          <button className="px-3 py-1 rounded border hover:bg-gray-50" onClick={() => { if (confirm("Excluir este lançamento? Essa ação não pode ser desfeita.")) onDelete(venda.id); }}>
            Excluir
          </button>
        </div>
      </td>
...
const Carteira: React.FC = () => {
  ...
  const [form, setForm] = useState<Partial<Venda>>({ cpf: "", data_venda: new Date().toISOString().slice(0,10), produto: "Automóvel", administradora: "Embracon", forma_venda: "Parcela Cheia", tipo_venda: "Normal", descricao: "", grupo: "", tabela: "", data_nascimento: "" });
  ...
  const [oferta, setOferta] = useState<Array<{administradora:string;grupo:string;cota:string|null;referencia?:string|null;participantes?:number|null;mediana?:number|null;contemplados?:number|null}>>([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 🔐 Admin: checagem mais robusta
        let adminFlag = false;
        try { const { data, error } = await supabase.rpc("is_admin_email", { e: uemail }); if (!error) adminFlag = !!data; } catch {}
        if (!adminFlag) { try { const { data } = await supabase.from("admins").select("email").eq("email", uemail).maybeSingle(); adminFlag = !!data; } catch {} }
        if (!adminFlag) { try { const { data } = await supabase.from("usuarios").select("email, perfil, role, app_metadata, user_metadata").eq("email", uemail).maybeSingle(); if (data) { const p=(data.perfil||data.role||"").toString().toLowerCase(); const roles = (data.app_metadata?.roles||[]).map((x:any)=>String(x).toLowerCase()); adminFlag = ["admin","administrador","adm","gestor","gestão","diretoria"].some(k=>p.includes(k)) || roles.includes("admin") || data.user_metadata?.is_admin === true; } } catch {} }
        ...
        if(!adminFlag){ pendQuery.eq("vendedor_id", uid); encQuery.eq("vendedor_id", uid); }
  ...
  const registrarVenda = async ()=>{
    try{
      ...
      const payload: Partial<Venda> = {
        ...
        segmento: segmento ?? undefined, data_nascimento: form.data_nascimento || null,
      ...
      const { data: pend } = await pendQuery; setPendentes(pend ?? []);
      setForm({ cpf:"", data_venda:new Date().toISOString().slice(0,10), produto:"Automóvel", administradora:"Embracon", forma_venda:"Parcela Cheia", tipo_venda:"Normal", descricao:"", grupo:"", tabela:"", data_nascimento:"" });
      setShowModal(false);
  ...
  const encarteirar = async (vendaId:string, grupo:string, cota:string, codigo:string)=>{
    try{
      ...
      const { error } = await supabase.from("vendas").update({ grupo, cota, codigo, status:"encarteirada", encarteirada_em: new Date().toISOString(), segmento: segmento ?? undefined }).eq("id", vendaId);
      if(error) throw error;

      const [{data:pend},{data:enc}] = await Promise.all([
        (async()=>{ const q=supabase.from("vendas").select("*").eq("status","nova").order("created_at",{ascending:false}); if(!isAdmin) q.eq("vendedor_id", userId); return await q; })(),
        (async()=>{ const q=supabase.from("vendas").select("*").eq("status","encarteirada").order("created_at",{ascending:false}); if(!isAdmin) q.eq("vendedor_id", userId); return await q; })(),
      ]);
      setPendentes(pend ?? []); setEncarteiradas(enc ?? []);
    }catch(e:any){ alert(e.message ?? "Erro ao encarteirar."); }
  };
  ...
  const salvarEdicao = async (v: Venda, patch: Partial<Venda>)=>{
    try{
      ...
      const { data: enc } = await encQuery; setEncarteiradas(enc ?? []);
  ...
  const salvarEdicaoPendente = async (venda: Venda, novo: Partial<Venda>)=>{
    try{
      ...
      if (patch.valor_venda != null) { const valor = Number(patch.valor_venda); if (Number.isNaN(valor)) throw new Error("Valor inválido."); patch.valor_venda = valor; }
   ...
   * ✅ Oferta de Lance
   * Agora a listagem é:
   * - Busca TODOS os grupos com assembleia = data (gestao_grupos_norm)
   * - Para cada grupo encontrado, cruza com as encarteiradas (respeitando o perfil)
   *   - Se houver cotas, cria 1 linha por cota
   *   - Se não houver cota, cria 1 linha com Cota = "—"
   */
  const listarOferta = async ()=>{
    try{
      // 1) pega os grupos da data na view normalizada
      const { data: gruposNorm, error } = await supabase
        .from("gestao_grupos_norm")
        .select("adm_norm,grupo_norm,assembleia_date,referencia,participantes,mediana,contemplados")
        .eq("assembleia_date", assembleia);
      if(error) throw error;

      // mapa p/ achar cotas por admin+grupo
      const encFonte = encarteiradas.filter(x=>!x.contemplada); // pode ter várias por grupo
      const buckets = new Map<string, Venda[]>(); // key = adm_norm::grupo_norm
      for(const v of encFonte){
        const key = `${(v.administradora||"").toLowerCase().trim()}::${(v.grupo||"").toString().trim()}`;
      ...
      for(const g of (gruposNorm ?? [])){
        const key = `${(g.adm_norm||"").toString().trim()}::${(g.grupo_norm||"").toString().trim()}`;
        const cotas = buckets.get(key) || [];

        if(cotas.length === 0){
          // grupo tem assembleia na data, porém não há cota encarteirada visível -> mostra mesmo assim
          linhas.push({
            administradora: g.adm_norm, // exibimos como veio; é normalizado (lower). Se preferir, pode capitalizar depois.
            grupo: g.grupo_norm,
            cota: null,
            referencia: (g.referencia && String(g.referencia).trim()!=="") ? g.referencia : null,
            participantes: g.participantes ?? null,
            mediana: g.mediana ?? null,
            contemplados: g.contemplados ?? null,
          });
        } else {
          // uma linha por cota existente
          for(const v of cotas){
            linhas.push({
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <table className="min-w-[860px] w-full border border-gray-200 rounded-xl">
            ...
            <tbody>
              ...
              {pendentesComNome.map(({venda,lead})=>(
                <LinhaEncarteirar key={venda.id} venda={venda} lead={lead} canEncarteirar={isAdmin} onSubmit={encarteirar} onDelete={excluirVenda} onOpenEditarVenda={(v)=>setEditVendaModal({open:true, venda:v})}/>
              ))}
            </tbody>
          </table>
        </div>
      ...
      {showCarteira && (
        <section className="space-y-3">
          ...
          {porCliente.map(group=>(
            <ClienteBloco key={group.cliente.id} group={group} onSaveVenda={salvarEdicao} isAdmin={isAdmin} onViewAllDescricoes={(title,itens)=>setDescModal({open:true,title,text: itens.map(x=>`Proposta ${x.proposta}: ${x.descricao || "—"}`).join("\n") || "Sem descrições"})}/>
          ))}
        </section>
      )}
      ...
      <section className="space-y-2">
        ...
        <div id="relatorio-oferta" className="overflow-auto">
          <table className="min-w-[880px] w-full border border-gray-200 rounded-xl">
            <thead className="bg-gray-50"><tr><th className="text-left p-2">Adm</th><th className="text-left p-2">Grupo</th><th className="text-left p-2">Cota</th><th className="text-left p-2">Referência</th><th className="text-left p-2">Participantes</th><th className="text-left p-2">Mediana</th><th className="text-left p-2">Contemplados</th></tr></thead>
            <tbody>
              {oferta.length===0 && (<tr><td className="p-3 text-gray-500" colSpan={7}>Nenhum grupo com assembleia nesta data.</td></tr>)}
              {oferta.map((o,idx)=>(
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl w-full max-w-3xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label className="text-sm text-gray-600">Data de Nascimento</label><input type="date" className="w-full border rounded-xl px-3 py-2" value={form.data_nascimento ?? ""} onChange={(e)=>onFormChange("data_nascimento", e.target.value)}/></div>
      ...
      {editVendaModal.open && editVendaModal.venda && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl w-full max-w-3xl p-5 space-y-4">
            <div className="flex items-center justify-between"><h3 className="text-xl font-semibold">Venda • {editVendaModal.venda.numero_proposta}</h3><button onClick={()=>setEditVendaModal({open:false})} className="text-gray-500 hover:text-gray-800">✕</button></div>
            {(() => {
              const v = editVendaModal.venda!;
              const [tmp, setTmp] = useState<Partial<Venda>>({
                lead_id: v.lead_id, cpf: v.cpf, data_venda: v.data_venda, produto: v.produto, administradora: v.administradora,
                forma_venda: v.forma_venda, numero_proposta: v.numero_proposta, valor_venda: v.valor_venda, tipo_venda: v.tipo_venda,
                descricao: v.descricao, tabela: v.tabela, grupo: v.grupo || "", data_nascimento: v.data_nascimento || "",
              });
              const Comp = () => (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className="text-sm text-gray-600">CPF</label><input className="w-full border rounded-xl px-3 py-2" value={formatCPF(tmp.cpf ?? "")} onChange={(e)=>setTmp(p=>({...p,cpf:e.target.value}))}/></div>
                    <div><label className="text-sm text-gray-600">Data da Venda</label><input type="date" className="w-full border rounded-xl px-3 py-2" value={tmp.data_venda ?? ""} onChange={(e)=>setTmp(p=>({...p,data_venda:e.target.value}))}/></div>
                    <div><label className="text-sm text-gray-600">Data de Nascimento</label><input type="date" className="w-full border rounded-xl px-3 py-2" value={tmp.data_nascimento ?? ""} onChange={(e)=>setTmp(p=>({...p,data_nascimento:e.target.value}))}/></div>
                    <div><label className="text-sm text-gray-600">Produto</label><select className="w-full border rounded-xl px-3 py-2" value={tmp.produto as Produto} onChange={(e)=>setTmp(p=>({...p,produto:e.target.value as Produto}))}>{PRODUTOS.map(p=><option key={p} value={p}>{p}</option>)}</select></div>
                    <div><label className="text-sm text-gray-600">Tabela</label><select className="w-full border rounded-xl px-3 py-2" value={tmp.tabela ?? ""} onChange={(e)=>setTmp(p=>({...p,tabela:e.target.value}))} disabled={(TABELAS[(tmp.produto as Produto)||"Automóvel"]||[]).length===0}><option value="">{(TABELAS[(tmp.produto as Produto)||"Automóvel"]||[]).length ? "Selecione a tabela…" : "Sem tabelas para este segmento"}</option>{(TABELAS[(tmp.produto as Produto)||"Automóvel"]||[]).map(t=><option key={t} value={t}>{t}</option>)}</select></div>
                    <div><label className="text-sm text-gray-600">Administradora</label><select className="w-full border rounded-xl px-3 py-2" value={tmp.administradora as Administradora} onChange={(e)=>setTmp(p=>({...p,administradora:e.target.value as Administradora}))}>{ADMINISTRADORAS.map(a=><option key={a} value={a}>{a}</option>)}</select></div>
                    <div><label className="text-sm text-gray-600">Forma da Venda</label><select className="w-full border rounded-xl px-3 py-2" value={tmp.forma_venda as FormaVenda} onChange={(e)=>setTmp(p=>({...p,forma_venda:e.target.value as FormaVenda}))}>{FORMAS.map(f=><option key={f} value={f}>{f}</option>)}</select></div>
                    <div><label className="text-sm text-gray-600">Número da Proposta</label><input className="w-full border rounded-xl px-3 py-2" value={tmp.numero_proposta ?? ""} onChange={(e)=>setTmp(p=>({...p,numero_proposta:e.target.value}))}/></div>
                    <div><label className="text-sm text-gray-600">Valor da Venda</label><input className="w-full border rounded-xl px-3 py-2" value={(tmp.valor_venda as any) ?? ""} onChange={(e)=>setTmp(p=>({...p,valor_venda:Number(e.target.value)}))} type="number" step="0.01"/></div>
                    <div className="md:col-span-2"><label className="text-sm text-gray-600">Descrição</label><textarea className="w-full border rounded-xl px-3 py-2" rows={3} value={tmp.descricao ?? ""} onChange={(e)=>setTmp(p=>({...p,descricao:e.target.value}))}/></div>
                  </div>
                  <div className="text-right mt-3">
                    <button className="px-4 py-2 rounded-xl border mr-2" onClick={()=>setEditVendaModal({open:false})}>Cancelar</button>
                    <button className="px-4 py-2 rounded-xl bg-[#1E293F] text-white hover:opacity-90" onClick={()=>salvarEdicaoPendente(v, tmp)}>Salvar</button>
                  </div>
                </>
              );
              return <Comp />;
