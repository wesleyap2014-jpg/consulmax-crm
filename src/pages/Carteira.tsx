...
type Venda = {
  ...
  tabela?: string | null;
  created_at: string;
  // campos auxiliares/novos (se existirem no banco serão gravados; senão, ignorados)
  segmento?: string | null;
  data_nascimento?: string | null; // ✅ Ponto 1
};
...
/** Normalização Produto -> Segmento (gravado em vendas.segmento) */
function normalizeProdutoToSegmento(
  produto: Produto | string | null | undefined
): string | null {
...
/** Linha Encarteirar (pendentes) */
type LinhaEncarteirarProps = {
  venda: Venda;
  lead?: Lead;
  canEncarteirar: boolean; // ✅ Ponto 5 (bloqueia para vendedor)
  onSubmit: (
    vendaId: string,
    grupo: string,
    cota: string,
  ...
  onViewEditarVenda: (venda: Venda) => void; // ✅ Ponto 2
};
      ...
      <td className="p-2">
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => {
              if (
                confirm("Excluir este lançamento? Essa ação não pode ser desfeita.")
              )
                onDelete(venda.id);
            }}
          >
...
const LinhaCota: React.FC<LinhaCotaProps> = ({
  venda,
  onSave,
  isAdmin,
  onViewDescricao,
  ...
  const [dataCont, setDataCont] = useState<string>(
    venda.data_contemplacao ?? ""
  );
  ...
  const saveContemplacao = async () => {
    ...
    await onSave({
      contemplada: flagCont,
      data_contemplacao: flagCont ? dataCont : null,
    });
  };
      ...
      <td className="p-2">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            onClick={() =>
              onViewDescricao(
                `Descrição - Proposta ${venda.numero_proposta}`,
                venda.descricao
              )
            }
          >
...
/** Bloco por cliente (encarteiradas) */
type ClienteGroup = {
  cliente: Lead;
  itens: Venda[];
  totalAtivas: number;
            ...
            <tbody>
              {group.itens.map((v) => (
                <LinhaCota
                  ...
                  onViewDescricao={(t, d) =>
                    onViewAllDescricoes(t, [
                      { proposta: v.numero_proposta, descricao: d },
                    ])
                  }
                  isAdmin={isAdmin}
                />
...
const Carteira: React.FC = () => {
  ...
  // Modal de Edição da venda pendente (👁️ do Encarteirar) — ✅ Ponto 2
  ...
  const [oferta, setOferta] = useState<
    Array<{
      ...
      mediana?: number | null; // ✅ Ponto 6 (trocamos “Maior % de LL” por “Mediana”)
          ...
          try {
            ...
            if (data) {
              const p = (data.perfil || data.role || "")
                .toString()
                .toLowerCase();
              adminFlag = ["admin", "administrador", "adm"].includes(p);
            }
          } catch {}
        }
  ...
  /** Filtragens por permissão — ✅ Pontos 3 e 4 */
  const pendentesVisiveis = useMemo(
    () => (isAdmin ? pendentes : pendentes.filter((v) => v.vendedor_id === userId)),
    [isAdmin, pendentes, userId]
  );
  const encarteiradasVisiveis = useMemo(
    () =>
      isAdmin
        ? encarteiradas
        : encarteiradas.filter((v) => v.vendedor_id === userId),
    [isAdmin, encarteiradas, userId]
  );

  const pendentesComNome = useMemo(
    () => pendentesVisiveis.map((v) => ({ venda: v, lead: leadMap[v.lead_id] })),
  ...
  const encarteiradasFiltradas = useMemo(() => {
    ...
    return encarteiradasVisiveis.filter((v) =>
      leadMap[v.lead_id]?.nome?.toLowerCase().includes(s)
    );
  }, [q, encarteiradasVisiveis, leadMap]);

  // ✅ Totais respeitam permissão (apenas o que o usuário pode ver)
  const totalAtivas = useMemo(
    () =>
      encarteiradasVisiveis.reduce(
  ...
  const registrarVenda = async () => {
    try {
      ...
      const payload: Partial<Venda> = {
        ...
        data_nascimento: form.data_nascimento || null, // ✅ Ponto 1
        vendedor_id: userId,
  ...
  const salvarEdicao = async (v: Venda, patch: Partial<Venda>) => {
    try {
      ...
      const { error } = await supabase
        .from("vendas")
        .update({ ...patch, segmento: seg ?? undefined })
        .eq("id", v.id);
      if (error) throw error;
      const { data: enc } = await supabase
        .from("vendas")
  ...
  // ✅ Edição da venda pendente (modal do “olhinho”)
  const salvarEdicaoPendente = async (venda: Venda, patch: Partial<Venda>) => {
    try {
      if (!isAdmin && venda.vendedor_id !== userId)
        throw new Error("Sem permissão para editar esta venda.");
      const seg = normalizeProdutoToSegmento(
        (patch.produto as Produto) ?? venda.produto
      );
      const { error } = await supabase
        .from("vendas")
        .update({ ...patch, segmento: seg ?? undefined })
  ...
  // ✅ Oferta de Lance — “Mediana” e sem depender de coluna específica para evitar erro
  const listarOferta = async () => {
    try {
      // Seleciona tudo para não quebrar se a coluna exata não existir
      const { data: grupos, error } = await supabase
        .from("gestao_grupos")
        .select("*")
        .eq("data_assembleia", assembleia as any);
      if (error) throw error;

      const map = new Map<string, any>();
      for (const g of (grupos as any[]) ?? []) map.set(`${g.administradora}::${g.grupo}`, g);
      ...
      for (const v of encarteiradasVisiveis.filter((x) => !x.contemplada)) {
        if (v.grupo && v.cota) {
          ...
          // tentativa de achar um campo de mediana com vários padrões possíveis
          const mediana =
            info.mediana ??
            info.mediana_ll ??
            info.mediana_perc_ll ??
            info.maior_perc_ll ?? // fallback se só existir este
            null;
          linhas.push({
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Gerencie vendas, encarteiramento e oferta de lance.
          </p>
        </div>
        <button
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500">
            {pendentesVisiveis.length} nova(s) venda(s)
          </span>
        </div>
            ...
            <tbody>
              ...
              {pendentesComNome.map(({ venda, lead }) => (
                <LinhaEncarteirar
                  ...
                  venda={venda}
                  lead={lead}
                  // ✅ vendedor NÃO pode encarteirar (Ponto 5)
                  canEncarteirar={isAdmin}
                  onSubmit={encarteirar}
                  onDelete={excluirVenda} // ✅ vendedor pode excluir
                  onViewEditarVenda={(v) => setEditVendaModal({ open: true, venda: v })} // ✅ Ponto 2
                />
              ))}
            </tbody>
          </table>
      ...
      {/* TOTAIS (respeitando permissão) — ✅ Ponto 3 */}
      <div className="flex items-center gap-4">
        ...
        <div className="px-4 py-3 rounded-2xl bg-gray-100">
          Canceladas:{" "}
          <strong className="ml-1">{currency(totalCanceladas)}</strong>
        </div>
              ...
              onViewAllDescricoes={(title, itens) =>
                setDescModal({
                  ...
                  text:
                    itens
                      .map(
                        (x) => `Proposta ${x.proposta}: ${x.descricao || "—"}`
                      )
                      .join("\n") || "Sem descrições",
                })
              }
      ...
      <section className="space-y-2">
        ...
        <div id="relatorio-oferta" className="overflow-auto">
          <table className="min-w-[720px] w-full border border-gray-200 rounded-xl grid">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left p-2">Adm</th>
                <th className="text-left p-2">Grupo</th>
                ...
                <th className="text-left p-2">Mediana</th> {/* ✅ Ponto 6 */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    onFormChange("data_nascimento", e.target.value)
                  }
                />
              </div>

              <div>
                <label className="text-sm text_gray-600">Vendedor</label>
                <input
                  className="w-full border rounded-xl px-3 py-2 bg-gray-50"
                  value={userName}
                  readOnly
              ...
              <div>
                ...
                <select
                  ...
                  disabled={
                    (TABELAS[(form.produto as Produto) || "Automóvel"] || [])
                      .length === 0
                  }
                >
                  <option value="">
                  ...
                  {(TABELAS[(form.produto as Produto) || "Automóvel"] || []).map(
                    (t) => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    onFormChange("forma_venda", e.target.value as FormaVenda)
                  }
                >
                  {FORMAS.map((f) => (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    onFormChange("numero_proposta", e.target.value)
                  }
                />
              </div>
      ...
      {/* Modal: Ver/Editar Vendas Pendentes — ✅ Ponto 2 */}
      {editVendaModal.open && editVendaModal.venda && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl w-full max-w-3xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-semibold">
                Editar Venda (Proposta {editVendaModal.venda.numero_proposta})
              </h3>
              <button
            ...
            {/* Form espelhado do “+ Nova Venda” com valores da venda */}
            <EditarVendaPendenteForm
              venda={editVendaModal.venda}
              leads={leads}
              leadMap={leadMap}
              onSalvar={(patch) => salvarEdicaoPendente(editVendaModal.venda!, patch)}
            />
          </div>
        </div>
      )}

      {/* Modal: Exibição de descrições */}
      {descModal.open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
...
/** Subcomponente do modal de edição da venda pendente — reusa os mesmos campos do cadastro */
const EditarVendaPendenteForm: React.FC<{
  ...
  const onChange = (k: keyof Venda, val: any) =>
    setLocal((f) => ({ ...f, [k]: val }));

  const tabelaOptions =
    TABELAS[(local.produto as Produto) || (venda.produto as Produto)] || [];
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            className="w-full border rounded-xl px-3 py-2 bg-gray-50"
            value={leadMap[local.lead_id as string]?.telefone ?? ""}
            readOnly
          />
        </div>
        ...
        <div>
          ...
          <select
            className="w-full border rounded-xl px-3 py-2"
            value={local.produto as Produto}
            onChange={(e) => onChange("produto", e.target.value as Produto)}
          >
            {PRODUTOS.map((p) => (
              <option key={p} value={p}>
                {p}
        ...
        <div>
          ...
          <select
            className="w-full border rounded-xl px-3 py-2"
            value={local.administradora as Administradora}
            onChange={(e) =>
              onChange("administradora", e.target.value as Administradora)
            }
          >
            {ADMINISTRADORAS.map((a) => (
              <option key={a} value={a}>
        ...
        <div>
          ...
          <select
            className="w-full border rounded-xl px-3 py-2"
            value={local.forma_venda as FormaVenda}
            onChange={(e) =>
              onChange("forma_venda", e.target.value as FormaVenda)
            }
          >
            {FORMAS.map((f) => (
              <option key={f} value={f}>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange("numero_proposta", e.target.value)}
          />
        </div>
        ...
        <div>
          ...
          <input
            className="w-full border rounded-xl px-3 py-2"
            value={(local.valor_venda as any) ?? ""}
            onChange={(e) => onChange("valor_venda", Number(e.target.value))}
            placeholder="R$ 0,00"
          />
        </div>
        ...
        <div>
          ...
          <select
            className="w-full border rounded-xl px-3 py-2"
            value={local.tipo_venda ?? "Normal"}
            onChange={(e) => onChange("tipo_venda", e.target.value)}
          >
            <option>Normal</option>
            <option>Contemplada</option>
            <option>Bolsão</option>
