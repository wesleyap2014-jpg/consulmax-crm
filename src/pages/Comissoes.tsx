...
function valorPorExtenso(n: number) {
  ...
  return `${ext(i)} ${i === 1 ? "real" : "reais"}${
    ct
      ? ` e ${ext(ct)} ${ct === 1 ? "centavo" : "centavos"}`
      : ""
  }`;
}
...
export default function ComissoesPage() {
  ...
  const [reciboImpostoPct, setReciboImpostoPct] = useState<string>("5"); // input em texto para aceitar vírgula
  ...
  const can = {
    viewVendasSem: isAdmin, // somente admin
    viewComissoesPagas: true, // ambos veem; vendedor só as dele
    managePayments: isAdmin, // registrar/estornar pagamentos
    viewDetalhamento: true, // ambos veem; vendedor só as dele
  };

  /* -------- Utilidades UI -------- */
  const impostoFrac = useMemo(() => {
    const n =
      parseFloat(String(reciboImpostoPct).replace(",", ".").trim()) / 100;
    ...
    try {
      ...
      if (comms && comms.length) {
        const { data: vendas } = await supabase
          .from("vendas")
          .select(
            "id, numero_proposta, cliente_lead_id, lead_id"
          )
      ...
      // vendas sem comissão (visível para admin)
      if (isAdmin) {
        ...
        const hasComm = new Set(
          (commVendaIds || []).map((r: any) => r.venda_id)
        );
  ...
  const rowsVisible = useMemo(() => {
    ...
    return rows.filter(
      (r) => (canonUserId(r.vendedor_id) || r.vendedor_id) === currentUserId
    );
  }, [rows, isAdmin, currentUserId]);

  const rowsAPagar = useMemo(
    () => rowsVisible.filter((r) => r.status === "a_pagar"),
    [rowsVisible]
  );
  ...
  const pagosFlat = useMemo(() => {
    ...
    return list.sort((a, b) =>
      (b.flow.data_pagamento_vendedor || "") >
  ...
  // Série anual (5 anos), mensal (ano atual), semanal (mês atual)
  type ProjSeries = { labels: string[]; previstoBruto: number[]; pagoBruto: number[] };

  const projectAnnualFlows = (
    ...
    for (const r of rs) {
      const total =
        r.valor_total ?? (r.base_calculo ?? 0) * (r.percent_aplicado ?? 0);
      const flows = (r.flow || []).filter((f) => (Number(f.percentual) || 0) > 0);
      for (const f of flows) {
        const expectedDate = addMonths(r.data_venda, (f.mes || 1) - 1);
        // ❗ Corrigido: evitar mistura de ?? com ||
        const expectedVal =
          f.valor_previsto ?? (total * (f.percentual ?? 0));
        if (expectedDate) {
      ...
      for (const f of flows) {
        ...
        if (
          ...
          const idx = intervals.findIndex(
            (iv) =>
              expectedDate.getTime() >= iv.start.getTime() &&
              expectedDate.getTime() <= iv.end.getTime()
          );
        ...
        if (f.data_pagamento_vendedor) {
          ...
          if (pd.getFullYear() === year && pd.getMonth() === month) {
            const idx2 = intervals.findIndex(
              (iv) =>
                pd.getTime() >= iv.start.getTime() &&
                pd.getTime() <= iv.end.getTime()
            );
  ...
  function totalsPaidAndProjectedInRange(s: Date, e: Date) {
    ...
    for (const r of rowsVisible) {
      ...
      for (const f of flows) {
        ...
        if (
          expectedDate &&
          expectedDate.getTime() >= s.getTime() &&
          expectedDate.getTime() <= e.getTime()
        ) {
      ...
      <div className="relative rounded-xl border bg-white p-3">
        ...
        {hover && hovered && (
          <div
            ...
            style={{
              left: Math.min(580, Math.max(8, 56 + hover.pi * xStep - 60)),
              top: 8,
            }}
          >
  ...
  async function paySelectedParcels(payload: {
    ...
    if (payload.recibo_file)
      reciboPath = await uploadToBucket(
        payload.recibo_file,
        payCommissionId
      );
    if (payload.comprovante_file)
      compPath = await uploadToBucket(
        payload.comprovante_file,
        payCommissionId
      );
    ...
    if (!selected.length && payload.data_pagamento_vendedor) {
      selected = candidates.filter(
        (f) =>
          (f.data_pagamento_vendedor || "") === payload.data_pagamento_vendedor
      );
    }
    if (!selected.length) {
      const unpaid = candidates.filter(
        (f) => (Number(f.valor_pago_vendedor) || 0) === 0
      );
  ...
  async function getNextReceiptNumber(): Promise<string> {
    // Tabela esperada: receipt_counter { id: 'main' pk, last_number: int }
    ...
    try {
      ...
      // fallback local (não reinicia build)
  ...
  async function downloadReceiptPDFPorData() {
    ...
    const impostoPct =
      parseFloat(String(reciboImpostoPct).replace(",", ".")) / 100 || 0;
    ...
    const vendaIds = Array.from(
      new Set((comms || []).map((c: any) => c.venda_id))
    );
    const { data: vendas } = await supabase
      .from("vendas")
      .select(
    ...
    const commsFiltradas = (comms || []).filter((c: any) =>
      reciboVendorId === "all" ? true : c.vendedor_id === reciboVendorId
    );
    ...
    const recebedor = [
      ...
      `Endereço: ${[
        ...
        vendInfo?.numero,
        vendInfo?.bairro,
        vendInfo?.cidade && `${vendInfo.cidade}/${vendInfo.uf}`,
      ]
        .filter(Boolean)
        .join(", ") || "—"}`,
    ];
    ...
    commsFiltradas.forEach((c: any) => {
      ...
      parcelas.forEach((p) => {
        const comBruta =
          (c.percent_aplicado || 0) * (p.percentual || 0) * vendaValor;
  ...
  return (
    <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <Select value={segmento} onValueChange={setSegmento}>
              ...
              <SelectContent>
                <SelectItem value="all">Todos</SelectItem>
                {Array.from(
                  new Set(simTables.map((t) => t.segmento))
                )
                  .filter(Boolean)
                  .map((seg) => (
          ...
          <div>
            ...
            <Select value={tabela} onValueChange={setTabela}>
              ...
              <SelectContent>
                ...
                {Array.from(
                  new Set(simTables.map((t) => t.nome_tabela))
                )
                  .filter(Boolean)
                  .map((tab) => (
      ...
      {/* ======== SEÇÕES: Relógio + Linhas (ordem solicitada) ======== */}
      {/* 1) Últimos 5 anos */}
      <Card>
        ...
        <CardContent className="space-y-3">
          <Donut
            paid={kpi5y.pagoLiquido}
            pending={kpi5y.pendente}
            label="5 anos"
          />
          <LineChart
            labels={ann.labels}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <Donut
            paid={kpiPrevY.pagoLiquido}
            pending={kpiPrevY.pendente}
            label="Ano anterior"
          />
          {/* Linha: filtra somente ano anterior */}
          {(() => {
            const prevMon = projectMonthlyFlows(
              rowsVisible.filter(
                (r) =>
                  new Date(r.data_venda || "").getFullYear() ===
      ...
      {/* 4) Mês Atual (semanas por quinta) */}
      <Card>
        <CardHeader className="pb-1">
          <CardTitle>
            Mês atual (semanas por quinta) — Previsto × Pago (bruto)
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
      ...
      {/* ======== KPIs (abaixo dos gráficos de linha) ======== */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            {BRL(
              sum(
                rowsVisible.map(
                  (r) =>
                    r.valor_total ??
                    (r.base_calculo ?? 0) * (r.percent_aplicado ?? 0)
                )
              )
            )}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="text-2xl font-bold">
            {BRL(kpiYear.pendente)}
          </CardContent>
        </Card>
      </div>

      {/* ======== Vendas sem comissão (sem Exportar CSV) — ADMIN APENAS ======== */}
      {can.viewVendasSem && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {/* (removido Exportar CSV conforme solicitado) */}
                {/* espaço reservado apenas para consistência visual */}
              </div>
            </CardTitle>
          </CardHeader>
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {/* Ordem: Vendedor > Data do Recibo > Imposto (%) Recibo > Ocultar */}
              <div>
                <Label>Vendedor</Label>
                <Select value={vendedorId} onValueChange={setVendedorId}>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs text-gray-500">
        Os arquivos são enviados ao bucket <code>comissoes</code>. Informe o
        valor <b>bruto</b>. Se nenhuma parcela estiver marcada, a confirmação faz
        seleção segura automática (1×100%).
      </div>
    </div>
  );
}
