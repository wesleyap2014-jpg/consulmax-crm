...
export default function ComissoesPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const setIds = new Set(commissionIds.map((_) => _));
  ...
  async function saveRule() {
    ...
    if (Math.abs(fluxoSoma - 1.0) > 0.00001) return alert("A soma do fluxo deve ser 1.0000 (100%).");
  ...
  async function gerarComissaoDeVenda(venda: Venda) {
    ...
    const insert = {
      ...
      // snapshots e cálculo serão preenchidos pelo trigger (mas mandamos se soubermos)
