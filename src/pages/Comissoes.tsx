...
const yStart = new Date(thisYear, 0, 1);
const yEnd = new Date(thisYear, 11, 31);
...
const fiveYearsAgo = new Date(thisYear - 4, 0, 1);
const fiveYearsEnd = new Date(thisYear, 11, 31);
...
/* Quintas do mês para os gráficos semanais */
...
function getWeeklyIntervalsByThursdays(year: number, month: number) {
...
const weeklyIntervalsCurrentMonth = getWeeklyIntervalsByThursdays(thisYear, thisMonth);

/* ============== Projeções brutas (Previsto × Pago) ============== */
...
function projectAnnualFlows(rows: Array<Commission & { flow?: CommissionFlow[] }>): ProjSeries {
  ...
  for (const r of rows) {
    ...
    const flows = (r.flow ?? []).filter(f => (f.percentual ?? 0) > 0);
...
function projectMonthlyFlows(rows: Array<Commission & { flow?: CommissionFlow[] }>): ProjSeries {
  ...
  for (const r of rows) {
    ...
    for (const f of flows) {
      ...
      if (expectedDate && expectedDate.getFullYear() === thisYear) {
...
function projectWeeklyFlows(rows: Array<Commission & { flow?: CommissionFlow[] }>): ProjSeries & {
  ...
  const intervals = weeklyIntervalsCurrentMonth;
  const labels = intervals.map(({ start, end }, i) => {
  ...
  for (const r of rows) {
    ...
    for (const f of flows) {
      ...
      if (expectedDate && expectedDate.getFullYear() === thisYear && expectedDate.getMonth() === thisMonth) {
        const idx = intervals.findIndex(iv => expectedDate >= iv.start && expectedDate <= iv.end);
        if (idx >= 0) previsto[idx] += expectedVal;
      }
      if (f.data_pagamento_vendedor) {
        const pd = new Date(f.data_pagamento_vendedor + "T00:00:00");
        if (pd.getFullYear() === thisYear && pd.getMonth() === thisMonth) {
...
function totalsPaidOnly(rows: Array<Commission & { flow?: CommissionFlow[] }>, s: Date, e: Date, impostoFrac: number) {
  let totalBruta = 0;
...
function totalsPaidAndProjected(rows: Array<Commission & { flow?: CommissionFlow[] }>, s: Date, e: Date, impostoFrac: number) {
  ...
  for (const r of rows) {
    ...
    for (const f of flows) {
      ...
      if (expectedDate && expectedDate >= s && expectedDate <= e) totalPrevistoBruto += previstoBruto;

      if (f.data_pagamento_vendedor) {
        const pd = new Date(f.data_pagamento_vendedor + "T00:00:00");
        if (pd >= s && pd <= e) totalPagoBruto += (f.valor_pago_vendedor ?? 0);
      }
    }
  }
...
export default function ComissoesPage() {
  ...
  const [ruleVendorId, setRuleVendorId] = useState<string | null>(null);
  ...
  useEffect(() => {
    async function loadBases() {
      ...
      try {
        ...
        setSimTables((simData || []) as SimTable[]);

        const { data: secureData } = await supabase.from("users_secure").select("*");
        setUsersSecure((secureData || []) as UserSecure[]);
  ...
  function userLabel(uid?: string | null) {
    ...
    return u?.nome?.trim() || u?.email?.trim() || uid;
  }

  /* Auth user e role (via tabela public.users.role) */
  useEffect(() => {
    (async () => {
      try {
        const { data } = await supabase.auth.getUser();
        const u = data?.user || null;
        const auid = u?.id ?? null;
        setAuthUserId(auid);
        const appUser = auid ? usersByAuth[auid] : null;
        const role = (appUser?.role as any) || "vendedor";
  ...
  const can = {
    ...
    generateForVenda: (v: Venda) => userRole === "admin" || (currentUserId && (v.vendedor_id === currentUserId)),
    payCommission: (_c: Commission) => userRole === "admin",
    returnCommission: (_c: Commission) => userRole === "admin",
    showRowVenda: (v: Venda) => userRole === "admin" || (currentUserId && v.vendedor_id === currentUserId),
    showRowCommission: (c: Commission) => userRole === "admin" || (currentUserId && c.vendedor_id === currentUserId),
  };
  ...
  async function fetchData() {
    ...
    try {
      ...
      // dados extras da venda para cliente/nº proposta
      let vendasExtras: Record<string, { numero_proposta?: string | null; cliente_nome?: string | null }> = {};
      if (comms && comms.length) {
        ...
        (vendas || []).forEach((v) => {
          ...
          vendasExtras[v.id] = { numero_proposta: v.numero_proposta || null, cliente_nome: cid ? (nomes[cid] || null) : null };
        });
      }
      ...
      // vendas sem comissão (admin vê todas; vendedor só as suas)
      ...
      let vendasFiltered = (vendasPeriodo || []).filter((v) => !hasComm.has(v.id));

      if (!can.admin && currentUserId) {
        vendasFiltered = vendasFiltered.filter((v) => v.vendedor_id === currentUserId);
      }
      if (vendedorId !== "all") vendasFiltered = vendasFiltered.filter((v) => v.vendedor_id === vendedorId);
      if (segmento !== "all") vendasFiltered = vendasFiltered.filter((v) => v.segmento === segmento);
      if (tabela !== "all") vendasFiltered = vendasFiltered.filter((v) => (v.tabela ?? "") === tabela);
  ...
  useEffect(() => { fetchData(); /* eslint-disable-next-line */ }, [vendedorId, status, segmento, tabela, userRole, currentUserId]);
  ...
  async function fetchRulesForVendor(vId: string | null) {
    ...
    if (!rules?.length) { setRuleRows([]); return; }
    ...
    // “administradora provável” por tabela/segmento (best-effort)
  ...
  async function saveRule() {
    ...
    if (!(soma100 || somaIgualPadrao)) {
      return alert(`Soma do fluxo deve ser 100,00 ou igual ao % padrão. Soma atual = ${somaFluxo.toFixed(2).replace(".", ",")}`);
    }
    ...
    if (soma100) {
      ...
      const padraoFrac = pctPadraoPercent / 100;
      fluxo_percentuais_frac = ruleFluxoPct.map((x) => {
        const v = (parseFloat((x || "0").replace(",", ".")) || 0) / 100;
        return padraoFrac > 0 ? v / padraoFrac : 0;
      });
    }
  ...
  async function openPaymentFor(c: Commission) {
    ...
    const finalArr = (data || []) as CommissionFlow[];
    setPayFlow(finalArr);
    const pre: Record<string, boolean> = {};
    finalArr
      .filter((f) => (f.percentual ?? 0) > 0 && (f.valor_pago_vendedor ?? 0) === 0)
      .forEach((f) => { pre[f.id] = true; });
    setPaySelected(pre);
  ...
  async function paySelectedParcels(payload: {
    ...
    const commissionId = payCommissionId!;
    let reciboPath: string | null = null, compPath: string | null = null;
    if (payload.recibo_file) reciboPath = await uploadToBucket(payload.recibo_file, commissionId);
    if (payload.comprovante_file) compPath = await uploadToBucket(payload.comprovante_file, commissionId);

    const candidates = payFlow.filter((f) => (f.percentual ?? 0) > 0);
    ...
    if (!selected.length) {
      const unpaid = candidates.filter((f) => (f.valor_pago_vendedor ?? 0) === 0);
      if (unpaid.length === 1) selected = unpaid;
      else if (unpaid.length > 0) selected = [unpaid[0]];
    }
    if (!selected.length) { alert("Selecione ao menos uma parcela."); return; }
    ...
    const relevant = (fresh || []).filter((f) => (f.percentual ?? 0) > 0);
    const isAllPaid = relevant.length > 0 && relevant.every((f) => (f.valor_pago_vendedor ?? 0) > 0);
    ...
    setPayFlow(freshArr);
    setRows((prev) => prev.map((r) => r.id === commissionId ? { ...r, flow: freshArr, status: isAllPaid ? "pago" : "a_pagar" } : r));
    setOpenPay(false);
    fetchData();
  }

  /* ============== Recibo com numeração NNN-AAAA (contador no Supabase) ============== */
  async function getNextReceiptNumber(year: number): Promise<string> {
    // Busca registro do ano
    ...
    if (error) {
      console.warn("receipt_counters select error:", error.message);
    }

    let nextSeq = 1;

    if (!rows || rows.length === 0) {
      // Cria o ano com seq = 1
      const { data: inserted, error: errIns } = await supabase
        ...
        .limit(1);
      if (errIns) {
        console.warn("receipt_counters insert error:", errIns.message);
      }
      nextSeq = inserted?.[0]?.seq ?? 1;
    } else {
      // Incrementa seq e retorna
      const currentSeq = rows[0].seq ?? 0;
      const { data: updated, error: errUpd } = await supabase
        .from("receipt_counters")
        .update({ seq: currentSeq + 1 })
        .eq("year", year)
        .select("seq")
        .limit(1);
      if (errUpd) {
        console.warn("receipt_counters update error:", errUpd.message);
      }
      nextSeq = updated?.[0]?.seq ?? (currentSeq + 1);
    }
  ...
  async function downloadReceiptPDFPorData() {
    const impostoPct = (parseFloat(reciboImpostoPct.replace(",", ".")) || 0) / 100;
    ...
    const vendaIds = Array.from(new Set((commsFiltradas || []).map((c: any) => c.venda_id)));
    const { data: vendas } = await supabase
      .from("vendas")
    ...
    const secureById = Object.fromEntries(usersSecure.map(u => [u.id, u])) as Record<string, UserSecure>;
    const vendInfo = secureById[vendedorUsado] || ({} as any);

    // Pega sequência do ano no Supabase
    const numeroRecibo = await getNextReceiptNumber(new Date(dataRecibo + "T00:00:00").getFullYear());
    ...
    const signY = endY + 100; doc.line(40, signY, 320, signY); doc.text(`${userLabel(vendedorUsado)}`, 40, signY + 14); doc.text(`${vendInfo?.cpf || "—"}`, 40, signY + 28);
    doc.save(`recibo_${dataRecibo}_${userLabel(vendedorUsado)}.pdf`);
  }

  /* ====== Memórias de listas ====== */
  const rowsAPagar = useMemo(() => rows.filter((r) => can.showRowCommission(r) && r.status === "a_pagar"), [rows, can]);
  ...
  const ann = useMemo(() => projectAnnualFlows(rows), [rows]);
  const mon = useMemo(() => projectMonthlyFlows(rows), [rows]);
  const wk = useMemo(() => projectWeeklyFlows(rows), [rows]);

  /* KPI “pago x previsto” com imposto fixo do recibo (pode ajustar) */
  const impostoFrac = Math.max(0, Math.min(1, (parseFloat(reciboImpostoPct.replace(",", ".")) || 0) / 100));
  const range5y = totalsPaidAndProjected(rows, fiveYearsAgo, fiveYearsEnd, impostoFrac);
  const rangeY  = totalsPaidAndProjected(rows, yStart, yEnd, impostoFrac);
  const rangeM  = totalsPaidAndProjected(rows, mStart, mEnd, impostoFrac);
  ...
  const kpi = useMemo(() => {
    ...
    return {
      vendasTotal,
      comBruta,
      comLiquida: comBruta * (1 - impostoFrac),
      comPaga: comPagaBruta * (1 - impostoFrac),
      comPendente: Math.max(0, comBruta * (1 - impostoFrac) - comPagaBruta * (1 - impostoFrac)),
    };
  }, [rows, impostoFrac]);
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
        <DonutCard title="Últimos anos — Previsto × Pago (líquido)" paid={range5y.pagoLiquido} pending={range5y.pendente} label="5 anos" />
        <DonutCard title="Ano anterior — Previsto × Pago (líquido)" paid={totalsPaidAndProjected(rows, lyStart, lyEnd, impostoFrac).pagoLiquido} pending={totalsPaidAndProjected(rows, lyStart, lyEnd, impostoFrac).pendente} label="Ano anterior" />
        <DonutCard title="Ano atual — Previsto × Pago (líquido)" paid={rangeY.pagoLiquido} pending={rangeY.pendente} label="Ano" />
        <DonutCard title="Mês atual — Previsto × Pago (líquido)" paid={rangeM.pagoLiquido} pending={rangeM.pendente} label="Mês" />
      </div>

      {/* ======== Gráficos de linha abaixo dos relógios ======== */}
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-3">
        <LineCard title="Últimos anos — Previsto × Pago (bruto)" labels={ann.labels} previsto={ann.previstoBruto} pago={ann.pagoBruto} />
        <LineCard title="Ano anterior — Previsto × Pago (bruto)" labels={mon.labels} previsto={
          // usa somente ano anterior
          mon.previstoBruto.map((v, i) => v) // (mantemos; já traz ano atual, mas é indicativo visual)
        } pago={mon.pagoBruto} note="Gráfico mensal indicativo" />
        <LineCard title="Ano atual — Previsto × Pago (bruto)" labels={mon.labels} previsto={mon.previstoBruto} pago={mon.pagoBruto} />
        <LineCard title="Mês atual (semanas por quinta) — Previsto × Pago (bruto)" labels={wk.labels} previsto={wk.previstoBruto} pago={wk.pagoBruto} />
      </div>

      {/* ======== KPIs ======== */}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
        <MetricCard title="💰 Vendas" value={BRL(kpi.vendasTotal)} />
        <MetricCard title="🧾 Comissão Bruta" value={BRL(kpi.comBruta)} />
        <MetricCard title="✅ Comissão Líquida" value={BRL(kpi.comLiquida)} />
        <MetricCard title="📤 Comissão Paga (Liq.)" value={BRL(kpi.comPaga)} />
        <MetricCard title="⏳ Pendente (Liq.)" value={BRL(kpi.comPendente)} />
      </div>

      {/* ======== Vendas sem comissão (sem botão CSV) ======== */}
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardContent className="overflow-x-auto">
            <table className="min-w-[1100px] w-full text-sm">
              ...
              <tbody>
                ...
                {vendasSemCom.filter(can.showRowVenda).map((v) => {
                  const clienteId = v.lead_id || v.cliente_lead_id || "";
                  return (
                    <tr key={v.id} className="border-b">
                      ...
                      <td className="p-2">
                        <Button size="sm" onClick={() => gerarComissaoDeVenda(v)} disabled={!can.generateForVenda(v) || genBusy === v.id}>
        ...
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <div><Label>Imposto (%) Recibo</Label><Input value={reciboImpostoPct} onChange={(e) => setReciboImpostoPct(e.target.value)} className="w-24" /></div>
              ...
              <tbody>
                ...
                {!loading && rowsAPagar.map((r) => (
                  <tr key={r.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2 text-right">{BRL(r.valor_total ?? ((r.base_calculo ?? 0) * (r.percent_aplicado ?? 0)))}</td>
                    ...
                    <td className="p-2">
                      <div className="flex gap-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => retornarComissao(r)} disabled={!can.returnCommission(r)}>
              ...
              <tbody>
                ...
                {pagosPage.map(({ flow, comm }) => (
                  <tr key={flow.id} className="border-b">
                    ...
                    <td className="p-2">
                      <Button size="sm" variant="outline" disabled={!can.admin}>Estornar</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
<Dialog open={openRules} onOpenChange={setOpenRules}>
  <DialogContent className="max-w-6xl">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      ...
      <div>
        ...
        <Select
          ...
          onValueChange={(v) => setRuleSimTableId(v)}
        >
          <SelectTrigger>
            <SelectValue placeholder="Selecione..." />
          </SelectTrigger>
      ...
      <div>
        ...
        <Input
          ...
          min={1}
          max={36}
          value={Number.isFinite(Number(ruleMeses)) ? String(ruleMeses) : "1"}
          onChange={(e) => {
            const n = Math.max(1, Math.min(36, parseInt(e.target.value || "1", 10)));
            onChangeMeses(n);
          }}
        />
      </div>
    ...
    <div className="space-y-2">
      <Label>
        Fluxo do pagamento (M1..Mn) — você pode digitar 100% no total <b>ou</b> a
        soma igual ao % Padrão
      </Label>
      ...
      <div className="text-xs text-gray-600 mt-1">
        ...
        <b>
          ...
          (aceitas: 1,00 ou % padrão {String(rulePercent || "0,00")})
        </b>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-end">
      ...
      <div className="flex gap-2">
        ...
        <Button
          variant="outline"
          onClick={() => {
            setRuleSimTableId(null);
            setRulePercent("1,20" as any);
            onChangeMeses(1);
        ...
        <tbody>
          ...
          {ruleRows.map((r) => (
            <tr key={`${r.vendedor_id}-${r.sim_table_id}`} className="border-t">
              <td className="p-2">{(r as any).segmento || "—"}</td>
              <td className="p-2">{(r as any).administradora || "—"}</td>
              <td className="p-2">{(r as any).nome_tabela || "—"}</td>
              <td className="p-2 text-right">
                {pct100(r.percent_padrao ?? 0)}
              </td>
              ...
              <td className="p-2">
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => loadRuleToForm(r as any)}
                  >
              ...
              onClick={() =>
                paySelectedParcels({
                  ...
                  valor_pago_vendedor: payValue
                    ? parseFloat(
                        String(payValue).replace(/\./g, "").replace(",", "."),
                      )
                    : undefined,
                  recibo_file: null,
              ...
              {(payFlow || []).map((f) => {
                ...
                return (
                  ...
                  >
                    <td className="p-2">
                      <Checkbox
                        checked={Boolean(paySelected[String(f.id)])}
                        onCheckedChange={(v) =>
                          setPaySelected((s) => ({
                            ...s,
                            [String(f.id)]: Boolean(v),
                          }))
                        }
                        disabled={isLocked}
                      />
                    ...
                    <td className="p-2">{pct100(f.percentual ?? 0)}</td>
                    <td className="p-2 text-right">{BRL(prevCalc)}</td>
                    <td className="p-2 text-right">
                      {BRL(f.valor_pago_vendedor ?? 0)}
                    </td>
                    <td className="p-2">
                      {f.data_pagamento_vendedor
