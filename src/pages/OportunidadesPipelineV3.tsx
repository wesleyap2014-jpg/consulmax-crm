// src/pages/OportunidadesPipelineV3.tsx
...
export default function OportunidadesPipelineV3() {
  ...
  const closedBySegment = useMemo(() => {
    const keys = new Set<string>();
    for (const o of [...wonOpps, ...lostOpps]) keys.add(o.segmento || "Sem segmento");
    const ordered = [...segmentos, ...Array.from(keys).filter((s) => !segmentos.includes(s))].filter((s, i, arr) => keys.has(s) && arr.indexOf(s) === i);
    return ordered.map((seg) => {
      const ganhos = wonOpps.filter((o) => (o.segmento || "Sem segmento") === seg);
      const perdidos = lostOpps.filter((o) => (o.segmento || "Sem segmento") === seg);
      return {
        segmento: seg,
        ganho: ganhos.length,
        perdido: perdidos.length,
        valorGanho: ganhos.reduce((a, o) => a + Number(o.valor_credito || o.credito_desejado || 0), 0),
        valorPerdido: perdidos.reduce((a, o) => a + Number(o.valor_credito || o.credito_desejado || 0), 0),
      };
    });
  }, [wonOpps, lostOpps]);
  ...
  async function createLead() {
    ...
    const { data: lead, error } = await supabase
      .from("leads")
      .insert({
        nome: leadForm.nome.trim(), telefone: onlyDigits(leadForm.telefone) || null, email: leadForm.email.trim().toLowerCase() || null,
        origem: leadForm.origem || null, descricao: leadForm.descricao || null, owner_id: meId,
      })
      .select()
      .single();
  ...
  async function createOpp() {
    ...
    setSaving(true);
    const payload = {
      lead_id: oppForm.lead_id, vendedor_id: vendedor, owner_id: vendedor, segmento: oppForm.segmento,
      valor_credito: parseBRL(oppForm.valor_credito), score: Number(oppForm.score || 1), estagio: dbStage(oppForm.estagio),
  ...
  async function saveEditing() {
    ...
    const patch: Partial<Opp> = {
      segmento: editing.segmento, valor_credito: Number(editing.valor_credito || 0), score: Number(editing.score || 1),
      expected_close_at: editing.expected_close_at || null, credito_desejado: Number(editing.credito_desejado || 0),
      parcela_desejada: Number(editing.parcela_desejada || 0), lance_disponivel: Number(editing.lance_disponivel || 0),
      prazo_contemplacao: editing.prazo_contemplacao || null, finalidade_recurso: editing.finalidade_recurso || null,
      reuniao_at: editing.reuniao_at || null, reuniao_tipo: editing.reuniao_tipo || null, reuniao_link: editing.reuniao_link || null,
      proposta_id: editing.proposta_id || null, fechamento_previsto_em: editing.fechamento_previsto_em || null, documentos_pendentes: editing.documentos_pendentes || null,
    };
    const saved = await updateOpp(editing, patch, newNote, "manual");
    if (saved) { setEditing(null); setNewNote(""); }
  }

  async function moveStage(op: Opp, target: StageUI) {
    if (target === "fechado_perdido") {
      setLostModal(op); setLostReason(op.lost_reason || "Não respondeu"); setLostDetails(op.lost_details || ""); return;
    }
    if (target === "fechado_ganho") { setWonModal(op); return; }
  ...
  async function confirmLost() {
    ...
    setLostModal(null); setLostDetails("");
  }
  ...
  async function scheduleMeeting(op: Opp) {
    ...
    const { error } = await supabase.from("agenda_eventos").insert({
      tipo: "reuniao", titulo: `Reunião • ${lead?.nome || "Lead"}`, lead_id: op.lead_id, user_id: op.vendedor_id,
      inicio_at: op.reuniao_at, fim_at: end, origem: "manual", opportunity_id: op.id, meeting_link: op.reuniao_link || null,
  ...
  function openEdit(op: Opp) {
    setEditing({ ...op }); setNewNote(""); loadProposalsForLead(op.lead_id);
  }
    ...
    <div style={pageStyle}>
      ...
      <header style={headerStyle}>
        <div>
          ...
          <p style={subtitleStyle}>Esteira comercial com paginação por estágio e resultado fechado por segmento.</p>
        </div>
      ...
      <section style={graphGridV3}>
        ...
        <div style={chartCardWide}>
          <div style={chartTitle}>Fechado Ganho x Fechado Perdido por segmento</div>
          {closedBySegment.length ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={closedBySegment}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="segmento" tick={{ fontSize: 11 }} />
                <YAxis allowDecimals={false} />
                <Tooltip formatter={(v: any, n: any, p: any) => {
                  const valueLabel = n === "ganho" ? "Ganhos" : "Perdidos";
                  const total = n === "ganho" ? p.payload.valorGanho : p.payload.valorPerdido;
                  return [`${v} • ${fmtBRLCompact(total)}`, valueLabel];
                }} />
                <Legend />
                <Bar dataKey="ganho" name="Fechado Ganho" fill={C.navy} radius={[8, 8, 0, 0]} />
                <Bar dataKey="perdido" name="Fechado Perdido" fill={C.red} radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : <div style={emptyChart}>Sem fechados no filtro atual.</div>}
        </div>
      </section>

      {loading ? <div style={loadingStyle}>Carregando oportunidades…</div> : (
        <main style={boardStyleV3}>
          {activeStages.map((stage) => {
            ...
            return (
              ...
              >
                ...
                <div style={columnTotal}>{fmtBRLCompact(allRows.reduce((a, o) => a + Number(o.valor_credito || o.credito_desejado || 0), 0))}</div>
...
const graphGridV3: React.CSSProperties = { position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1.45fr", gap: 14, marginBottom: 16 };
...
const chartCardWide: React.CSSProperties = { ...chartCard };
...
const boardStyleV3: React.CSSProperties = { position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "repeat(5, minmax(270px, 1fr))", gap: 12, overflowX: "auto", paddingBottom: 18 };
