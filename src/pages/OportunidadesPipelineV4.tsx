// src/pages/OportunidadesPipelineV4.tsx
...
const donutColors = [C.red, C.navy, C.gold, C.goldLight, "#8B1F2A", "#2B3B5A", "#9C8B58", "#F5E9B6"];
...
export default function OportunidadesPipelineV4() {
  ...
  const stageData = useMemo(() => {
    return activeStages.map((s) => {
      ...
      return { stage: s, label: stageLabels[s], qtd: rows.length, total: rows.reduce((a, o) => a + moneyBase(o), 0) };
    });
  }, [openPipeline]);
  ...
  const setPageFor = (stage: StageUI, page: number, totalPages: number) => {
    setStagePages((p) => ({ ...p, [stage]: Math.min(Math.max(1, page), Math.max(1, totalPages)) }));
  };
    ...
    <div style={pageStyle}>
      ...
      <header style={headerStyle}>
        <div>
          ...
          <p style={subtitleStyle}>Esteira comercial com paginação por estágio e donuts de fechados por segmento.</p>
        </div>
      ...
      <section style={filterCard}>
        ...
        <select style={inputStyle} value={tempFilter} onChange={(e) => setTempFilter(e.target.value)}>
          <option value="__all__">Todas temperaturas</option><option value="quente">Quente</option><option value="morno">Morno</option><option value="frio">Frio</option>
        </select>
        <select style={inputStyle} value={dueFilter} onChange={(e) => setDueFilter(e.target.value)}>
          <option value="__all__">Todas datas</option><option value="late">Atrasadas</option><option value="today">Hoje</option><option value="nodate">Sem data</option>
        </select>
      </section>

      <section style={graphGridV4}>
        <div style={chartCard}>
        ...
        <DonutSegmentCard title="Fechado Ganho por segmento" rows={wonBySegment} totalLabel="ganho" empty="Sem ganhos no filtro atual." />
        <DonutSegmentCard title="Fechado Perdido por segmento" rows={lostBySegment} totalLabel="perdido" empty="Sem perdas no filtro atual." />
      </section>

      {loading ? <div style={loadingStyle}>Carregando oportunidades…</div> : (
        <main style={boardStyleV4}>
          {activeStages.map((stage) => {
            const allRows = byStage(stage);
            const totalPages = Math.max(1, Math.ceil(allRows.length / PAGE_SIZE));
            const current = Math.min(pageFor(stage), totalPages);
            const rows = allRows.slice((current - 1) * PAGE_SIZE, current * PAGE_SIZE);
            return (
              <section
                key={stage}
              ...
              >
                ...
                <div style={pagerStyle}>
                  <button style={pagerBtn} disabled={current <= 1} onClick={() => setPageFor(stage, current - 1, totalPages)}>‹</button>
                  <span style={pagerText}>{current}/{totalPages}</span>
                  <button style={pagerBtn} disabled={current >= totalPages} onClick={() => setPageFor(stage, current + 1, totalPages)}>›</button>
                </div>
              </section>
            );
          })}
        ...
        <Modal title={`Tratar oportunidade • ${leadMap.get(editing.lead_id)?.nome || "Lead"}`} onClose={() => setEditing(null)} wide>
          <div style={modalGrid}>
            <div style={modalSection}>
              ...
              <select style={inputStyle} value={normalizeStage(editing.estagio)} onChange={(e) => moveStage(editing, e.target.value as StageUI)}>{allStages.map((s) => <option key={s} value={s}>{stageLabels[s]}</option>)}</select>
              ...
              <label style={labelStyle}>Valor do crédito</label><MoneyInput value={brlInputFromNumber(editing.valor_credito)} onChange={(v) => setEditing({ ...editing, valor_credito: parseBRL(v) })} />
              <label style={labelStyle}>Score / Temperatura</label><select style={inputStyle} value={String(editing.score || 1)} onChange={(e) => setEditing({ ...editing, score: Number(e.target.value) })}>{[1,2,3,4,5].map((n) => <option key={n} value={n}>{n} • {tempLabel(n).label}</option>)}</select>
              <label style={labelStyle}>Previsão de fechamento</label><input style={inputStyle} type="date" value={editing.expected_close_at || ""} onChange={(e) => setEditing({ ...editing, expected_close_at: e.target.value })} />
            </div>
            ...
            <div style={modalSection}>
              ...
              <label style={labelStyle}>Prazo pretendido para contemplação</label><input style={inputStyle} value={editing.prazo_contemplacao || ""} onChange={(e) => setEditing({ ...editing, prazo_contemplacao: e.target.value })} />
            ...
            <div style={modalSection}>
              ...
              <div style={timelineStyle}>{(noteMap.get(editing.id) || []).map((n) => <div key={n.id} style={timelineItem}><strong>{fmtDateTimeBR(n.created_at)} • {n.kind}</strong><p>{n.note}</p></div>)}{!(noteMap.get(editing.id) || []).length && <div style={emptyCol}>Sem anotações ainda.</div>}</div>
            </div>
          </div>
...
function groupBySegment(rows: Opp[]) {
  const map = new Map<string, { name: string; value: number; total: number }>();
  for (const o of rows) {
    const key = o.segmento || "Sem segmento";
    const prev = map.get(key) || { name: key, value: 0, total: 0 };
    prev.value += 1;
    prev.total += moneyBase(o);
    map.set(key, prev);
  }
  const ordered = [...segmentos, ...Array.from(map.keys()).filter((s) => !segmentos.includes(s))];
  return ordered.map((s) => map.get(s)).filter(Boolean) as { name: string; value: number; total: number }[];
}

function DonutSegmentCard({ title, rows, totalLabel, empty }: { title: string; rows: { name: string; value: number; total: number }[]; totalLabel: string; empty: string }) {
  const totalQtd = rows.reduce((a, r) => a + r.value, 0);
  const totalValor = rows.reduce((a, r) => a + r.total, 0);
  return (
    <div style={chartCard}>
      ...
      <div style={donutWrap}>
          ...
          <>
            <div style={{ height: 180 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={rows} dataKey="value" nameKey="name" innerRadius={48} outerRadius={74} paddingAngle={3}>
                    {rows.map((_, i) => <Cell key={i} fill={donutColors[i % donutColors.length]} />)}
                  </Pie>
                  <Tooltip formatter={(v: any, _n: any, p: any) => [`${v} ${totalLabel}(s) • ${fmtBRLCompact(p.payload.total)}`, p.payload.name]} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div style={donutCenterInfo}>
              <strong>{totalQtd}</strong>
              <span>{fmtBRLCompact(totalValor)}</span>
            </div>
            <div style={legendList}>
              {rows.map((r, i) => (
                <div key={r.name} style={legendItem}>
                  <span style={{ ...legendDot, background: donutColors[i % donutColors.length] }} />
        ...
        ) : <div style={emptyChart}>{empty}</div>}
      </div>
    </div>
  );
}
...
function OppCard({ op, lead, vendor, notes, onEdit, onDragStart, whatsapp }: { op: Opp; lead?: Lead; vendor?: string; notes: Note[]; onEdit: () => void; onDragStart: (e: React.DragEvent<HTMLDivElement>) => void; whatsapp: string }) {
  ...
  return <div style={cardStyle} draggable onDragStart={onDragStart}><div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}><strong style={{ color: C.navy }}>{lead?.nome || "Lead não localizado"}</strong><span style={{ ...tagStyle, background: `${u.color}18`, color: u.color }}>{u.label}</span></div><div style={muted}>{formatPhoneBR(lead?.telefone)} • {vendor || "—"}</div><div style={cardMoney}>{fmtBRLCompact(op.valor_credito || op.credito_desejado)}</div><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><span style={{ ...tagStyle, background: `${t.color}16`, color: t.color }}>{t.label}</span><span style={tagStyle}>{op.segmento || "—"}</span></div><p style={lastNote}>{last.length > 120 ? `${last.slice(0, 120)}...` : last}</p><div style={{ display: "flex", gap: 8, marginTop: 10 }}><button style={miniBtn} onClick={onEdit}>Tratar</button>{whatsapp && <a style={miniBtnLink} href={whatsapp} target="_blank" rel="noreferrer">WhatsApp</a>}</div></div>;
}
...
const graphGridV4: React.CSSProperties = { position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr .85fr .85fr", gap: 14, marginBottom: 16 };
...
const donutWrap: React.CSSProperties = { position: "relative" };
const donutCenterInfo: React.CSSProperties = { position: "absolute", top: 72, left: "50%", transform: "translateX(-50%)", display: "grid", placeItems: "center", color: C.navy, pointerEvents: "none" };
const legendList: React.CSSProperties = { display: "grid", gap: 6, marginTop: 6, maxHeight: 96, overflow: "auto" };
const legendItem: React.CSSProperties = { display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: C.ink2 };
const legendDot: React.CSSProperties = { width: 10, height: 10, borderRadius: 999 };
const boardStyleV4: React.CSSProperties = { position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "repeat(5, minmax(270px, 1fr))", gap: 12, overflowX: "auto", paddingBottom: 18 };
