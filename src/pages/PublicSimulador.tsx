 ...
 * Integra com RPCs SECURITY DEFINER:
 *  - public_create_opportunity
 *  - public_update_opportunity_stage
 * Campos-alvo na view `public.oportunidades`: segmento, valor_credito, observacao, estagio.
 */
...
export default function PublicSimulador() {
  ...
  // WhatsApp: tenta falar direto com o número do usuário; senão cai no número oficial da Consulmax
  ...
  async function handleSimular() {
    ...
    try {
      ...
      // ✅ cria oportunidade via RPC (security definer) — grava em oportunidades_base e reflete na view
      const { data, error } = await supabase.rpc("public_create_opportunity", {
        p_nome: nome.trim(),
        p_email: email.trim(),
        p_telefone: telefone,
        p_segmento: modalidade,                                                  // mapeia para `segmento`
        p_tipo_simulacao: tipoSimulacao,                                         // vai dentro de `observacao`
        ...
        p_prazo_meses: prazo ? Number(prazo) : null,                             // `observacao`
        p_administradora: admin,                                                 // `observacao`
        p_anotacoes: `[${ts()}] ${anot}`,                                        // `observacao`
      });
            ...
            <CardContent className="grid gap-5">
              <div className="rounded-xl p-4 bg-white border border-[#1E293F]/10">
                <p className="text-sm text-[#1E293F]/80">
                  Sua simulação foi registrada. Um especialista pode te contatar para refinar a proposta ideal para você. Enquanto isso, escolha uma opção
                  abaixo:
                </p>
              </div>
