...
function TableFormOverlay({
  ...
  const [taxaAdmPct, setTaxaAdmPct] = useState<number>(initial?.taxa_adm_pct ?? 0.22);
  const [frPct, setFrPct] = useState<number>(initial?.fundo_reserva_pct ?? 0.02);
  const [antecipPct, setAntecipPct] = useState<number>(initial?.antecip_pct ?? 0.02);
  const [antecipParcelas, setAntecipParcelas] = useState<number>(initial?.antecip_parcelas ?? 1);
  const [limPct, setLimPct] = useState<number>(initial?.limitador_parcela_pct ?? 0.002565);
  const [seguroPct, setSeguroPct] = useState<number>(initial?.seguro_prest_pct ?? 0.00061);
  ...
  async function salvar() {
    ...
    const payload: Omit<SimTable, "id"> = {
      ...
      limitador_parcela_pct: limPct,
      seguro_prest_pct: seguroPct,
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-lg">
        ...
        <div className="p-4 grid gap-3 md:grid-cols-4">
          ...
          <div>
            ...
            <PercentInput valueDecimal={taxaAdmPct} onChangeDecimal={setTaxaAdmPct} placeholder="ex.: 22,0000" />
          </div>
          <div>
            <Label>% Fundo Reserva</Label>
            <PercentInput valueDecimal={frPct} onChangeDecimal={setFrPct} placeholder="ex.: 2,0000" />
          </div>
          <div>
            <Label>% Antecipação da Adm</Label>
            <PercentInput valueDecimal={antecipPct} onChangeDecimal={setAntecipPct} placeholder="ex.: 2,0000" />
          </div>
          ...
          <div>
            ...
            <PercentInput valueDecimal={limPct} onChangeDecimal={setLimPct} placeholder="ex.: 0,2565" />
          </div>
          <div>
            <Label>% Seguro por parcela</Label>
            <PercentInput valueDecimal={seguroPct} onChangeDecimal={setSeguroPct} placeholder="ex.: 0,0610" />
          </div>
    ...
    <div className="space-y-6">
      ...
      {p.leadId ? (
        <>
          ...
          {/* Configurações do Lance (sem exibir teto/limite aqui) */}
          <Card>
              ...
              {lanceModoParcela ? (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">O limite do embutido é aplicado automaticamente pelas regras da administradora.</p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">O limite é aplicado pela base configurada na administradora.</p>
                  </div>
                </>
              )}
