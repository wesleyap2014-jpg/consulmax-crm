...
function calcularSimulacao(i: CalcInput): CalcResult {
  ...
  for (let mes = 1; mes <= mCont; mes++) {
    ...
    extrato.push({
      ...
      evento: reaj !== 0 ? `Reajuste aniversário: ${(indexPct*100).toFixed(2)}%` : undefined,
    });
  }
  ...
  extrato.push({
    ...
    saldoAposPagamento: saldo,
    investimentoAcumulado: investimentoAcum,
    evento: `Lance ofertado: ${brMoney(lanceOfertadoValor)} | Embutido: ${brMoney(lanceEmbutidoValor)}`,
  });
  ...
  while (mesAtual <= prazo && saldo > 0.01) {
    ...
    if (isAniver) {
      ...
      extrato.push({
        ...
        saldoAposPagamento: saldo,
        investimentoAcumulado: investimentoAcum,
        evento: `Reajuste aniversário (pós): ${(indexPct*100).toFixed(2)}%`,
      });
    }
...
function ExtratoModal({
  ...
  function exportarExtratoPDF() {
    ...
    (doc as any).autoTable({
      head: [["Parcela","Crédito","Valor Pago","Reajuste","Saldo devedor","Investimento acumulado","Evento"]],
      body: rows,
