...
function isEmbracon(admin: AdminRow) {
  const slug = adminSlug(admin);
  const name = (admin.name || "").toLowerCase().trim();

  return slug === "embracon" || name === "embracon";
}
...
function descriptionFor(admin: AdminRow) {
  ...
  if (isEmbracon(admin)) {
    return "Simulador exclusivo para regras Embracon: crédito, parcela cheia, reduzida, lance embutido, lance próprio e pós-contemplação.";
  }
...
export default function Simuladores() {
  ...
  useEffect(() => {
    ...
    async function loadAdmins() {
      ...
      if (full.error) {
        // Compatibilidade caso o banco ainda não tenha logo_url/description.
        ...
        if (fallback.error) {
          console.error(
            "Erro ao carregar administradoras:",
        ...
        setAdmins((full.data ?? []) as AdminRow[]);
      }
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      >
        ...
        <div
          className="absolute -bottom-24 left-12 h-56 w-56 rounded-full blur-3xl"
        ...
        <div className="relative z-[1] max-w-3xl text-white">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-medium backdrop-blur">
            ...
            Hub de Simuladores Consulmax
          </div>

          <h1 className="text-2xl md:text-4xl font-black tracking-tight">
            Escolha a administradora para iniciar a simulação
          </h1>

          <p className="mt-3 text-sm md:text-base text-white/82">
            Cada administradora terá seu próprio arquivo e sua própria regra de
            cálculo. Assim, a Embracon fica blindada e novas administradoras
            entram sem quebrar o que já funciona.
          </p>
        </div>
      </section>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {orderedAdmins.map((admin) => {
          const available = isEmbracon(admin);
          ...
          return (
            ...
            >
              <CardContent className="p-5 space-y-5">
                ...
                <p className="min-h-[72px] text-sm leading-relaxed text-slate-600">
                ...
                <div className="rounded-2xl border bg-slate-50/80 p-3 text-xs text-slate-600">
                  ...
                  >
                    ...
                    Estrutura isolada por administradora
                  </div>

                  <p className="mt-1">
                    Arquivo próprio, cálculo próprio e evolução sem interferir
                    nos demais simuladores.
                  </p>
                </div>
                ...
                <Button
                  ...
                  style={{
                    background: available ? C.ruby : C.navy,
                    color: "white",
                  }}
        ...
        {orderedAdmins.length === 0 && (
          <Card className="rounded-[28px] border bg-white/75 shadow-sm backdrop-blur md:col-span-2 xl:col-span-3">
            <CardContent className="p-8 text-center text-sm text-slate-600">
              Nenhuma administradora cadastrada em{" "}
              <strong>sim_admins</strong>.
            </CardContent>
          </Card>
        )}
      </div>
