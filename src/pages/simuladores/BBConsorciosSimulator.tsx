...
export default function BBConsorciosSimulator() {
  ...
  const [selectedRange, setSelectedRange] = useState<CreditRange | null>(null);
  ...
  useEffect(() => {
    ...
    if (Math.round(Number(selectedRange.valor || 0) * 100) !== Math.round(creditoAtual * 100)) {
      setSelectedRange(null);
    }
  }, [creditoInput, selectedRange]);

  const creditoDigitado = parseMoney(creditoInput);
  const credito = creditoDigitado;
