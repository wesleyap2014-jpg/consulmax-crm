...
function normalizeConfig(group?: BBGroup | null): BBConfig {
  ...
  const prazoRules: PrazoRule[] =
    Array.isArray(raw.prazoRules) && raw.prazoRules.length
      ? raw.prazoRules.map((r: any) => ({
          ...
          fundoReservaPct: Number(
            r.fundoReservaPct ?? group?.fundo_reserva_pct ?? 0
          ),
        }))
      : [
          {
  ...
  if (Array.isArray(raw.lanceOptions)) {
    raw.lanceOptions.forEach((l: any) => {
      if (l?.key && byKey.has(l.key)) {
        byKey.set(l.key, {
          ...
          nomeComercial: String(
            l.nomeComercial || LANCE_LABELS[l.key as LanceKey]
          ),
          pct: Number(l.pct || 0),
        });
      }
    ...
    byKey.set("livre", {
      key: "livre",
      enabled: group?.permite_lance_livre !== false,
      nomeComercial: "Lance Livre",
      pct: 0,
    });

    byKey.set("primeiro_fixo", {
      key: "primeiro_fixo",
      enabled: group?.permite_fixo_25 !== false,
      nomeComercial: "Fixo 25%",
      pct: 0.25,
    });

    byKey.set("segundo_fixo", {
      key: "segundo_fixo",
      enabled: group?.permite_fixo_50 !== false,
      nomeComercial: "Fixo 50%",
      pct: 0.5,
    });
  }
  ...
  return {
    ...
    regraPosContemplacao:
      (raw.regraPosContemplacao as RegraPos) ||
      "saldo_devedor_prazo_restante",
...
function defaultForm(segmento: SegmentoBB): GroupForm {
  return {
    ...
    lanceOptions: DEFAULT_LANCES.map((l) => ({
      ...l,
...
function payloadFromForm(form: GroupForm) {
  const creditRanges = form.creditRanges
    .map((r) => ({
      id: r.id || makeId("cred"),
      label: r.label.trim() || formatMoneyInput(parseMoney(r.valor)),
  ...
  const prazoRules = form.prazoRules
    .map((r) => ({
      ...
      prazo: Math.max(1, numberFrom(r.prazo)),
  ...
  const taxaAdmMedia = prazoRules.length
    ? prazoRules[prazoRules.length - 1].taxaAdmPct
    : 0;

  const fundoReservaMedia = prazoRules.length
    ? prazoRules[prazoRules.length - 1].fundoReservaPct
    : 0;
  ...
  return {
    ...
    permite_lance_livre:
      lanceOptions.find((l) => l.key === "livre")?.enabled ?? true,
    permite_lance_embutido: permiteLanceEmbutido,
    lance_embutido_max_pct: permiteLanceEmbutido
      ? parsePercent(form.maxLanceEmbutidoPct)
      : 0,
    ...
    config: {
      ...
      prazoRules,
      lanceOptions,
      maxLanceEmbutidoPct: permiteLanceEmbutido
        ? parsePercent(form.maxLanceEmbutidoPct)
        : 0,
...
function calcBB(input: {
  ...
  if (!group) {
    return { result: null as any, error: "Selecione um grupo/tabela BB." };
  }

  if (!credito) {
    return {
      result: null as any,
      error: "Selecione ou informe o crédito desejado.",
    };
  }

  const cfg = normalizeConfig(group);

  const creditValues = cfg.creditRanges
    .map((r) => Number(r.valor || 0))
    .filter((v) => v > 0);

  const min = creditValues.length
    ? Math.min(...creditValues)
    : Number(group.credito_min || 0);

  const max = creditValues.length
    ? Math.max(...creditValues)
    : Number(group.credito_max || 0);

  if (min > 0 && credito < min) {
    return {
      result: null as any,
      error: `Crédito abaixo da faixa mínima deste grupo: ${brMoney(min)}.`,
    };
  }
  ...
  if (max > 0 && credito > max) {
    return {
      result: null as any,
      error: `Crédito acima da faixa máxima deste grupo: ${brMoney(max)}.`,
    };
  }

  const sortedPrazoRules = [...cfg.prazoRules].sort(
    (a, b) => a.prazo - b.prazo
  );
  ...
  const nearestRule =
    exactRule ||
    sortedPrazoRules.find((r) => r.prazo >= prazo) ||
    sortedPrazoRules[sortedPrazoRules.length - 1];

  const prazoMin = sortedPrazoRules.length
    ? Math.min(...sortedPrazoRules.map((r) => r.prazo))
    : Number(group.prazo_min || 1);

  const prazoMax = sortedPrazoRules.length
    ? Math.max(...sortedPrazoRules.map((r) => r.prazo))
    : Number(group.prazo_max || prazo || 1);

  const prazoFinal = clamp(
    Math.max(1, prazo || prazoMax),
    prazoMin || 1,
    prazoMax || prazo || 1
  );

  const taxaAdm = Number(nearestRule?.taxaAdmPct ?? group.taxa_adm_pct ?? 0);
  const fundoReserva = Number(
    nearestRule?.fundoReservaPct ?? group.fundo_reserva_pct ?? 0
  );
  const seguroPct = Number(group.seguro_pct || 0);

  const permiteEmbutido = group.permite_lance_embutido !== false;
  const maxEmbutidoPct = permiteEmbutido
    ? Number(cfg.maxLanceEmbutidoPct || 0)
    : 0;
  ...
  const lanceOption = cfg.lanceOptions.find(
    (l) => l.key === lanceKey && l.enabled
  );

  if (!lanceOption) {
    return {
      result: null as any,
      error: "A modalidade de lance selecionada não está habilitada para este grupo.",
    };
  }
  ...
  const parcelaContempl = clamp(
    Math.max(1, parcelaContemplacao || 1),
    1,
    prazoFinal
  );

  const prazoRestanteAposContemplacao = Math.max(
    1,
    prazoFinal - parcelaContempl
  );
  ...
  if (usarEmbutido) {
    if (!permiteEmbutido) {
      return {
        result: null as any,
        error: "Este grupo não permite lance embutido.",
      };
    }
    ...
    if (maxEmbutidoPct > 0 && embutidoPctFinal > maxEmbutidoPct) {
      return {
        result: null as any,
        error: `Lance embutido acima do máximo permitido (${pctHuman(
          maxEmbutidoPct
        )}).`,
      };
    }
  ...
  const lanceProprioValor = Math.max(
    0,
  ...
  const saldoAposPagamentos = Math.max(
    0,
    valorCategoria - totalPagoAteContemplacao
  );

  const saldoAposLance = Math.max(0, saldoAposPagamentos - lanceOfertadoValor);

  let parcelaApos =
    saldoAposLance / prazoRestanteAposContemplacao + seguroMensal;

  let prazoRestanteFinal = prazoRestanteAposContemplacao;

  if (
    cfg.regraPosContemplacao === "mantem_parcela_reduz_prazo" &&
    parcelaAntes > 0
  ) {
    prazoRestanteFinal = Math.max(
      1,
      Math.ceil(saldoAposLance / Math.max(1, parcelaAntes - seguroMensal))
    );

    parcelaApos = parcelaAntes;
  }

  const investimentoAteContemplacao =
    parcelaAntes * parcelaContempl + lanceProprioValor;
...
function ConfigOverlay({
  ...
  groups: BBGroup[];
  onEditGroup: (group: BBGroup | null) => void;
  onDeleteGroup: (group: BBGroup) => Promise<void>;
  onSaved: () => Promise<void>;
}) {
  const [form, setForm] = useState<GroupForm>(() =>
    defaultForm(initialSegmento)
  );
  ...
  function addCreditRange() {
    setForm((f) => ({
      ...
      creditRanges: [
        ...f.creditRanges,
        { id: makeId("cred"), label: "", valor: formatMoneyInput(0) },
      ],
    }));
  }
  ...
  async function save() {
    ...
    if (!(payload.config as BBConfig).creditRanges.length) {
      setError("Cadastre pelo menos uma faixa de crédito.");
      return;
    }

    if (!(payload.config as BBConfig).prazoRules.length) {
      setError("Cadastre pelo menos uma regra de prazo/taxa.");
      return;
    }

    if (!(payload.config as BBConfig).lanceOptions.some((l) => l.enabled)) {
      setError("Habilite pelo menos uma modalidade de lance.");
      return;
    }
      ...
      <div className="max-h-[92vh] w-full max-w-6xl overflow-auto rounded-[28px] border bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white/95 p-4 backdrop-blur">
          <div>
            ...
            <div className="text-xs text-slate-500">
              Configure faixas de crédito, prazos, taxas e lances permitidos por grupo.
            </div>
          </div>
        ...
        <div className="grid gap-4 p-5 md:grid-cols-2">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  segmento: e.target.value as SegmentoBB,
                }))
              }
            >
              {segmentos.map((s) => (
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Ex.: BB Auto IPCA 80"
            />
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, nome_grupo: e.target.value }))
              }
              placeholder="Ex.: BB Consórcio Auto IPCA"
            />
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, seguro_pct: e.target.value }))
              }
          ...
          <div className="md:col-span-2 rounded-[24px] border bg-slate-50/70 p-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Mesmo padrão do Maggi: cadastre as faixas/valores disponíveis para o grupo.
                </div>
              </div>
            ...
            <div className="space-y-2">
              {form.creditRanges.map((range, idx) => (
                <div
                  key={range.id}
                  className="grid gap-2 md:grid-cols-[1fr_220px_44px]"
                >
                  <Input
                    value={range.label}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        creditRanges: f.creditRanges.map((r) =>
                          r.id === range.id
                    ...
                    placeholder="Nome da faixa. Ex.: 100 mil"
                  />
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setForm((f) => ({
                        ...f,
                        creditRanges: f.creditRanges.filter(
                          (_, i) => i !== idx
                        ),
                      }))
                    }
          ...
          <div className="md:col-span-2 rounded-[24px] border bg-slate-50/70 p-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Cada prazo pode ter taxa de administração e fundo de reserva próprios.
                </div>
              </div>
            ...
            <div className="space-y-2">
              {form.prazoRules.map((rule, idx) => (
                <div
                  key={rule.id}
                  className="grid gap-2 md:grid-cols-[120px_1fr_1fr_44px]"
                >
                  <Input
                    value={rule.prazo}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        prazoRules: f.prazoRules.map((r) =>
                          r.id === rule.id
                            ? { ...r, prazo: e.target.value }
                            : r
                        ),
                      }))
                    }
                  ...
                  <Input
                    value={rule.taxaAdmPct}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        prazoRules: f.prazoRules.map((r) =>
                          r.id === rule.id
                            ? { ...r, taxaAdmPct: e.target.value }
                            : r
                        ),
                      }))
                    }
                  ...
                  <Input
                    value={rule.fundoReservaPct}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        prazoRules: f.prazoRules.map((r) =>
                          r.id === rule.id
                            ? { ...r, fundoReservaPct: e.target.value }
                            : r
                        ),
                      }))
                    }
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setForm((f) => ({
                        ...f,
                        prazoRules: f.prazoRules.filter((_, i) => i !== idx),
                      }))
                    }
                  >
                    <Trash2 className="h-4 w-4" />
          ...
          <div className="md:col-span-2 rounded-[24px] border bg-slate-50/70 p-4">
            <div className="mb-3">
              ...
              <div className="text-xs text-slate-500">
                Habilite/desabilite modalidades e configure o percentual quando for lance fixo/limitado/fidelidade.
              </div>
            </div>
            ...
            <div className="space-y-2">
              {form.lanceOptions.map((lance) => (
                <div
                  key={lance.key}
                  className="grid gap-2 rounded-2xl border bg-white p-3 md:grid-cols-[180px_1fr_180px] md:items-center"
                >
                  <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                    <input
                      type="checkbox"
                      checked={lance.enabled}
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          lanceOptions: f.lanceOptions.map((l) =>
                            l.key === lance.key
                  ...
                  <Input
                    value={lance.nomeComercial}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        lanceOptions: f.lanceOptions.map((l) =>
                          l.key === lance.key
                            ? { ...l, nomeComercial: e.target.value }
                            : l
                        ),
                      }))
                    }
                  ...
                  <Input
                    value={lance.pct}
                    disabled={lance.key === "livre"}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        lanceOptions: f.lanceOptions.map((l) =>
                          l.key === lance.key
                            ? { ...l, pct: e.target.value }
                            : l
                        ),
                      }))
                    }
          ...
          <div className="md:col-span-2 rounded-[24px] border bg-slate-50/70 p-4">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <input
                type="checkbox"
                checked={form.permite_lance_embutido}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    permite_lance_embutido: e.target.checked,
                  }))
                }
              />
              Este grupo permite lance embutido?
            </label>

            {form.permite_lance_embutido && (
              <div className="mt-3 max-w-xs">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      maxLanceEmbutidoPct: e.target.value,
                    }))
                  }
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  regra_pos_contemplacao: e.target.value as RegraPos,
                }))
              }
            >
        ...
        <div className="border-t bg-slate-50/70 p-5">
          <div className="mb-3 flex items-center justify-between gap-2">
            <div>
              <div className="font-black" style={{ color: C.navy }}>
                Grupos cadastrados BB
              </div>
              <div className="text-xs text-slate-500">
                A gestão dos grupos fica concentrada aqui dentro do menu de configuração.
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {groups.map((g) => {
              ...
              const min = cfg.creditRanges.length
                ? Math.min(...cfg.creditRanges.map((r) => r.valor))
                : Number(g.credito_min || 0);

              const max = cfg.creditRanges.length
                ? Math.max(...cfg.creditRanges.map((r) => r.valor))
                : Number(g.credito_max || 0);

              return (
                <div
                  key={g.id}
                  className="flex flex-col gap-3 rounded-2xl border bg-white p-3 md:flex-row md:items-center md:justify-between"
                >
                  <div>
                    <div className="font-black" style={{ color: C.navy }}>
                      {g.nome_grupo || g.grupo}
                    </div>
                    <div className="text-xs text-slate-500">
                      {getSegmentoLabel(g.segmento)} • {brMoney(min)} a{" "}
                      {brMoney(max)} • {cfg.prazoRules.length} prazo(s) •
                      Embutido:{" "}
                      {g.permite_lance_embutido === false ? "Não" : "Sim"} •{" "}
                      {g.is_active === false ? "Inativo" : "Ativo"}
                    </div>
                  </div>
...
function GrupoTabela({ group }: { group: BBGroup | null }) {
  ...
  if (!group || !cfg.creditRanges.length || !cfg.prazoRules.length) {
    return null;
  }
    ...
    <Card className="rounded-[28px] border bg-white/78 shadow-sm backdrop-blur">
      ...
      <CardContent>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th className="p-3">Fundo reserva</th>
              ...
              {cfg.creditRanges.flatMap((range) =>
                prazos.map((prazo) => {
                  const valorCategoria =
                    range.valor * (1 + prazo.taxaAdmPct + prazo.fundoReservaPct);

                  const parcela =
                    prazo.prazo > 0 ? valorCategoria / prazo.prazo : 0;

                  return (
                    <tr key={`${range.id}_${prazo.id}`} className="border-t">
                      <td className="p-3 font-semibold text-slate-700">
                        {range.label || brMoney(range.valor)}
                      </td>
                      <td className="p-3">{brMoney(range.valor)}</td>
                      <td className="p-3">{prazo.prazo} meses</td>
                      <td className="p-3">{pctHuman(prazo.taxaAdmPct)}</td>
                      <td className="p-3">{pctHuman(prazo.fundoReservaPct)}</td>
        ...
        <div className="mt-2 text-xs text-slate-500">
          A parcela da tabela considera crédito + taxa de administração + fundo de reserva, sem seguro mensal e sem efeitos de lance.
        </div>
      </CardContent>
    </Card>
  );
...
export default function BBConsorciosSimulator() {
  ...
  const [userProfile, setUserProfile] = useState<LoggedUserProfile | null>(
    null
  );
  ...
  const [creditoManualInput, setCreditoManualInput] = useState(
    formatMoneyInput(0)
  );
  const [usarCreditoManual, setUsarCreditoManual] = useState(false);

  const [prazoInput, setPrazoInput] = useState("80");
  const [parcelaContemplacaoInput, setParcelaContemplacaoInput] =
    useState("12");
  ...
  async function load() {
    ...
    if (authUserId) {
      ...
      const role = String(
        (me as any)?.role || (me as any)?.user_role || ""
      ).toLowerCase();
  ...
  const availableLances = useMemo(
    () => selectedConfig.lanceOptions.filter((l) => l.enabled),
    [selectedConfig]
  );

  const availablePrazos = useMemo(
    () => [...selectedConfig.prazoRules].sort((a, b) => a.prazo - b.prazo),
    [selectedConfig]
  );
  ...
  useEffect(() => {
    if (
      selectedConfig.creditRanges.length &&
      !selectedConfig.creditRanges.some((r) => r.id === creditoSelecionado)
    ) {
      setCreditoSelecionado(selectedConfig.creditRanges[0].id);
    }

    if (
      availablePrazos.length &&
      !availablePrazos.some((p) => String(p.prazo) === prazoInput)
    ) {
      setPrazoInput(String(availablePrazos[availablePrazos.length - 1].prazo));
    }

    if (
      availableLances.length &&
      !availableLances.some((l) => l.key === lanceKey)
    ) {
      setLanceKey(availableLances[0].key);
    }

    if (!grupoPermiteEmbutido) {
    ...
    prazoInput,
    lanceKey,
    grupoPermiteEmbutido,
  ]);

  const creditoFromRange =
    selectedConfig.creditRanges.find((r) => r.id === creditoSelecionado)
      ?.valor || 0;

  const credito = usarCreditoManual
    ? parseMoney(creditoManualInput)
    : creditoFromRange;
  ...
  const calculation = calcBB({
    ...
    usarEmbutido: grupoPermiteEmbutido && usarEmbutido,
  ...
  if (loading) {
    return (
      <div className="p-6 flex items-center gap-2 text-sm text-slate-600">
        ...
        Carregando simulador BB Consórcios...
      </div>
    );
  }
        ...
        <div className="relative z-[1] flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div className="max-w-3xl text-white">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-medium backdrop-blur">
              ...
              Simulador BB Consórcios
            </div>

            <h1 className="text-2xl md:text-4xl font-black tracking-tight">
              Configure a estratégia de crédito e lance
            </h1>

            <p className="mt-3 text-sm md:text-base text-white/82">
              Simulação por grupo/tabela, com faixas de crédito, prazos, taxas e lances permitidos no padrão Maggi.
            </p>
          </div>
        ...
        <Card className="rounded-[28px] border border-amber-200 bg-amber-50/80 shadow-sm">
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center gap-2 font-bold text-amber-800">
              ...
              Tabela sim_bb_groups ainda não existe
            </div>

            <p className="text-sm text-amber-800">
              Rode este SQL no Supabase para habilitar o simulador BB:
            </p>

            <pre className="max-h-[320px] overflow-auto rounded-2xl bg-slate-950 p-4 text-xs text-white">
              {sqlForSetup()}
            </pre>
          </CardContent>
        </Card>
                ...
                {segmentos.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderColor: active ? C.ruby : "rgba(30,41,63,.12)",
                        background: active ? "rgba(161,28,39,.08)" : "white",
                      }}
                    >
                      <Icon
                        className="mb-2 h-5 w-5"
                        style={{ color: active ? C.ruby : C.navy }}
                      />
          ...
          <Card className="rounded-[28px] border bg-white/78 shadow-sm backdrop-blur">
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
                  ...
                  {segmentGroups.map((g) => {
                    ...
                    return (
                      <option key={g.id} value={g.id}>
                        {g.nome_grupo || g.grupo} • {brMoney(min)} a{" "}
                        {brMoney(max)}
                      </option>
                    );
                  })}
              ...
              <div>
                ...
                <select
                  className="h-10 w-full rounded-md border px-3"
                  value={usarCreditoManual ? "manual" : creditoSelecionado}
                  onChange={(e) => {
                    const v = e.target.value;
                    setUsarCreditoManual(v === "manual");

                    if (v !== "manual") {
                      setCreditoSelecionado(v);
                    }
                  }}
                >
                  {selectedConfig.creditRanges.map((r) => (
                    <option key={r.id} value={r.id}>
                      {r.label || brMoney(r.valor)} • {brMoney(r.valor)}
                    </option>
                  ))}
              ...
              {usarCreditoManual && (
                <div>
                  ...
                  <Input
                    value={creditoManualInput}
                    onChange={(e) =>
                      setCreditoManualInput(
              ...
              <div>
                ...
                >
                  {availablePrazos.map((p) => (
                    <option key={p.id} value={p.prazo}>
                      {p.prazo} meses • Adm {pctHuman(p.taxaAdmPct)} • FR{" "}
                      {pctHuman(p.fundoReservaPct)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <Input
                  value={parcelaContemplacaoInput}
                  onChange={(e) => setParcelaContemplacaoInput(e.target.value)}
                />
              </div>
              ...
              <div>
                ...
                >
                  {availableLances.map((l) => (
                    <option key={l.key} value={l.key}>
                      {l.nomeComercial}
                      {l.key !== "livre" ? ` • ${pctHuman(l.pct)}` : ""}
                    </option>
                  ))}
                </select>
              </div>
              ...
              {lanceKey === "livre" && (
                <div>
                  ...
                  <Input
                    value={lanceLivrePct}
                    onChange={(e) => setLanceLivrePct(e.target.value)}
              ...
              {grupoPermiteEmbutido ? (
                <div className="rounded-2xl border bg-slate-50/70 p-3">
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    Máximo do grupo: {pctHuman(selectedConfig.maxLanceEmbutidoPct)}
                  </div>
                </div>
              ) : (
                <div className="rounded-2xl border bg-slate-50/70 p-3 text-sm font-semibold text-slate-500">
                  Este grupo não permite lance embutido.
                </div>
              )}

              {grupoPermiteEmbutido && usarEmbutido && (
                <div>
                  <Label>Percentual do lance embutido</Label>
                  <Input
                    value={lanceEmbutidoPct}
                    onChange={(e) => setLanceEmbutidoPct(e.target.value)}
                    placeholder="Ex.: 25,0000"
                  />
                </div>
              )}
            ...
            <>
              <div className="grid gap-3 md:grid-cols-2">
                <Metric
                  ...
                  value={brMoney(result.credito)}
                  hint={`Líquido estimado: ${brMoney(result.creditoLiquido)}`}
                />

                <Metric
                  label="Parcela antes"
                  value={brMoney(result.parcelaAntes)}
                  hint={`Seguro mensal: ${brMoney(result.seguroMensal)}`}
                />

                <Metric
                  label="Lance ofertado"
                  value={brMoney(result.lanceOfertadoValor)}
                  hint={`${result.lanceNome} • ${pctHuman(result.lancePct)} sobre o crédito`}
                />

                <Metric
                  label="Lance próprio"
                  value={brMoney(result.lanceProprioValor)}
                  hint={`Embutido: ${brMoney(result.lanceEmbutidoValor)}`}
                />

                <Metric
                  label="Parcela pós-contemplação"
                  value={brMoney(result.parcelaApos)}
                  hint={`${result.prazoRestanteAposContemplacao} parcelas restantes`}
                />

                <Metric
                  label="Investimento até contemplação"
                  value={brMoney(result.investimentoAteContemplacao)}
                  hint={`Até a parcela ${result.parcelaContemplacao}`}
                />
              </div>
              ...
              <Card className="rounded-[28px] border bg-white/78 shadow-sm backdrop-blur">
                ...
                <CardContent className="space-y-3">
                  <textarea
                    className="min-h-[260px] w-full rounded-2xl border bg-slate-50 p-3 text-sm"
                    readOnly
                    value={resumoTexto}
                  />
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {buildWhatsappLink(userProfile) && (
                      <Button
                        ...
                        className="rounded-2xl"
                        onClick={() =>
                          window.open(buildWhatsappLink(userProfile), "_blank")
                        }
                      >
                        Abrir WhatsApp
                      </Button>
