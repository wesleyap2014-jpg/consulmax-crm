...
export default function WhatsAppAtendimento() {
  ...
  const counts = useMemo(() => {
    ...
    const estimatedOpenings = convs.filter((c) => !!c.last_message_at).length;
    const estimatedCostBRL = 0;
    return { open, closed, novos, won, lost, winRate, total: convs.length, inbound, outbound, estimatedOpenings, estimatedCostBRL };
  }, [convs, msgs]);
