...
const render = (tpl: string | null | undefined, values: Record<string,string>) => String(tpl || "").replace(/{{\s*([a-zA-Z0-9_]+)\s*}}/g, (_m, key) => values[key] ?? "");
const fmtDateLong = (iso: string) => new Intl.DateTimeFormat("pt-BR", { timeZone: TZ, weekday: "long", day: "2-digit", month: "long", year: "numeric" }).format(new Date(iso));
const fmtDateShort = (iso: string) => new Intl.DateTimeFormat("pt-BR", { timeZone: TZ, day: "2-digit", month: "2-digit", year: "numeric" }).format(new Date(iso));
const fmtTime = (iso: string) => new Intl.DateTimeFormat("pt-BR", { timeZone: TZ, hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(iso));
...
function makeIcs(event:any, guest:any, organizerEmail:string, organizerName:string) {
  ...
  const description = [event.descricao || "", meeting ? `Videoconferência: ${meeting}` : ""].filter(Boolean).join("\n\n");
  return [
    "BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//Consulmax CRM//Agenda//PT-BR","CALSCALE:GREGORIAN","METHOD:REQUEST","BEGIN:VEVENT",
    `UID:${event.id}@consulmaxcrm`,`DTSTAMP:${icsDate(new Date().toISOString())}`,`DTSTART:${icsDate(event.inicio_at)}`,`DTEND:${icsDate(end)}`,
    `SUMMARY:${icsText(event.titulo || "Compromisso Consulmax")}`,
    description ? `DESCRIPTION:${icsText(description)}` : "",
    meeting ? `URL:${meeting}` : "", meeting ? `LOCATION:${icsText("Videoconferência Consulmax")}` : "",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: guests, error: guestError }, { data: template, error: templateError }, { data: organizer }] = await Promise.all([
      ...
      admin.from("email_templates").select("template_key,name,version,subject_template,preheader_template,html_template,text_template,sender_name,reply_to").eq("template_key","agenda_invite").eq("is_active",true).order("version",{ascending:false}).limit(1).maybeSingle(),
    ...
    for (const guest of guests) {
      try {
        ...
        const plain:Record<string,string> = {
          titulo:title,nome_convidado:guestName,data_curta:fmtDateShort(event.inicio_at),data_extenso:fmtDateLong(event.inicio_at),hora_inicio:fmtTime(event.inicio_at),hora_fim:fmtTime(endIso),organizador_nome:organizerName,link_confirmar:yes,link_recusar:no,logo_url:logoUrl,preheader:"",descricao_texto:description,videoconferencia_texto:meeting?`Videoconferência: ${meeting}`:"",descricao_html:"",videoconferencia_html:""
        };
        plain.preheader = render(template.preheader_template, plain);
        const htmlValues:Record<string,string> = {
          ...plain,titulo:esc(title),nome_convidado:esc(guestName),data_curta:esc(plain.data_curta),data_extenso:esc(plain.data_extenso),hora_inicio:esc(plain.hora_inicio),hora_fim:esc(plain.hora_fim),organizador_nome:esc(organizerName),link_confirmar:esc(yes),link_recusar:esc(no),logo_url:esc(logoUrl),preheader:esc(plain.preheader),
          descricao_html:description?`<div style="margin-top:18px;padding-top:16px;border-top:1px solid #E2E8F0"><div style="font-size:12px;color:#64748B;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Observações</div><div style="margin-top:7px;font-size:14px;line-height:1.65;color:#475569">${esc(description).replace(/\r?\n/g,"<br>")}</div></div>`:"",
          videoconferencia_html:meeting?`<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#FFF9F9;border:1px solid #EBC7CA;border-radius:16px"><tr><td style="padding:18px 20px"><div style="font-size:12px;color:#A11C27;font-weight:800;text-transform:uppercase;letter-spacing:.06em">Videoconferência</div><div style="margin-top:5px;font-size:16px;color:#1E293F;font-weight:800">Sala online da reunião</div><a href="${esc(meeting)}" style="display:inline-block;margin-top:13px;background:#A11C27;color:#FFFFFF;text-decoration:none;font-size:14px;font-weight:800;padding:11px 17px;border-radius:10px">Acessar videoconferência</a></td></tr></table>`:""
        };
        ...
        await transporter.sendMail({
          ...
          headers:{"X-Consulmax-Template":`${template.template_key}; v=${template.version}`,"X-Consulmax-Mime":"calendar-then-html","X-Consulmax-App-Url":appUrl},
        ...
        console.log(`[agenda-invite] sent ${template.template_key} v${template.version} to guest ${guest.id} appUrl=${appUrl}`);
        sent++;
      } catch (e) {
    ...
    return json({ok:true,sent,failed,template_key:template.template_key,template_version:template.version,mime:"calendar-then-html",app_url:appUrl});
  } catch(e) {
